...
wms.Source = L.Layer.extend({
    ...
    'getEvents': function() {
        if (this.options.identify) {
            return {'contextmenu': this.identify};
        } else {
            return {};
        }
    },
    ...
    'refreshOverlay': function() {
		...
		var lyr = [];
		var obj = this._subLayers;
		Object.keys(obj).forEach(function(key) {
			if (obj[key]) lyr.push(key);
		});
		var subLayers = lyr.join(",");
        if (!this._map) {
            return;
        }
        if (!subLayers) {
    ...
    'getIdentifyLayers': function() {
		...
		//change method detect layer obj with status: true or false
    ...
    'parseFeatureInfo': function(result, url) {
        ...
        if (result == "error") {
            ...
            result = "<iframe src='" + url + "' style='border:none; min-height:60px; max-height:120px; height:100%'>";
        }
        return result;
    },
    ...
    'showFeatureInfo': function(latlng, info) {
        ...
        if (info) this._map.openPopup(info, latlng);
    },
