...
class akkm extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->parent_module_title="IPCC WGII";
		$this->module_title="LIST DATA AREA KONSERVATIF KELOLA MASYARAKAT";
		$this->tbl_idx="idx";
	 ...
	 function index(){
        ...
        if($this->input->get_post("tahun")):
			if($this->input->get_post("tahun")=="All"):
				...
				$where[]	=	" YEAR(tanggal)='".$this->input->get_post("tahun")."'";
			endif;
		else:
			$where[]	=	" YEAR(tanggal)='".date('Y')."'";
        endif;
		...
		if($this->input->get_post("status")):
            $where[]	=	" status_akkm='".$this->input->get_post("status")."'";
        endif;
		...
		$sortBy=" order by idx asc, tanggal asc";
	...
	function read_import(){
		if ($_SERVER['REQUEST_METHOD']=='POST'){
			...
			$uploadPath=$this->config->item('dir_import_akkm');
			...
			$inputFileName = $this->config->item('dir_import_akkm').$media['file_name'];
			...
			// for ($i=0; $i<$numSheets; $i++) {
				...
				foreach($arr_data as $k=>$v):
					if($v['B']=='' && $v['C']=='' && $v['D']=='' && $v['E']=='' && $v['G']=='' && $v['H']=='' && $v['K']=='' && $v['L']==''  && $v['M']=='') {
						 continue;
					}
					$datax[] = $v;
				endforeach;
	...
	function add_xls(){
		if($_SERVER['REQUEST_METHOD']=="POST"):
			...
			$coloumn_txt[0] 	="ID";
			$coloumn_txt[1] 	="nama_akkm";
			
			$coloumn_txt[2] 	="kd_kecamatan";
			$coloumn_txt[3] 	="kd_kabupaten";
			$coloumn_txt[4] 	="kd_propinsi";

			$coloumn_txt[5] ="longitude";
			$coloumn_txt[6] ="latitude";

			$coloumn_txt[7] 	="pengampu";
			$coloumn_txt[8] 	="luas";
			$coloumn_txt[9] 	="ekosistem";
			$coloumn_txt[10] 	="status_akkm";
			$coloumn_txt[11] 	="deskripsi";

			if($coloumn_txt):
				$col_txt = implode("|",$coloumn_txt);
			endif;
			...
			if(cek_array($postx)):
				...
				foreach($postx as $x=>$val):
					...
					if(cek_array($data_map)):
						$data_map['tanggal']=date("Y-m-d");
					
						$data_map=$this->_add_creator($data_map);
					endif;
	...
	public function get_export_template(){
		...
		if($tahun):
			if($tahun=="All"):
				$filter_table[]	=	"year(tanggal) <= '".date('Y')."'";
			else:
				$filter_table[]	=	"year(tanggal) = '".$tahun."'";
			endif;
		endif;
	 ...
	 function add(){
        ...
        if($act=="create"):
			...
			if($_FILES['file_peta']['name']!=''):
				...
				if ($id_last):
					//$sql = "UPDATE t_daftar_wikera SET the_geom=ST_GeomFromGeoJSON('".$json."',3), file_peta='".$datafile_peta."' WHERE idx='".$id_last."'";
					$sql = "UPDATE t_akkm SET the_geom='".$json."', file_peta='".$datafile_peta."' WHERE idx='".$id_last."'";
	...
	public function delete_geojson(){
		...
		//set value thegeom and file_peta empty
		$data["the_geom"]  = "";
		$data["file_peta"] = "";
		
	
		$dataz	=	$this->model->GetRecordData("idx='".$param['idx']."'");
		if(cek_array($dataz)):
			unlink($this->config->item("dir_geojson").$dataz['file_path']);
		endif;
		
		$updt = $this->model->UpdateData($data,"idx=$param[idx]");
