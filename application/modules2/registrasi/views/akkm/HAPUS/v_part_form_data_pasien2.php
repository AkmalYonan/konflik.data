    	...
    	<div class="row">
			<div class="col-md-6">
            	<div class="form-group">
                	<?php	$tgl_registrasi=$data["tgl_registrasi"]?$data["tgl_registrasi"]:date("Y-m-d H:i:s"); ?>
           	...
           	<div class="col-md-6">
            	<div class="form-group">
                    ...
                    <input class="form-control required" name="no_rekam_medis" id="no_rekam_medis" type="text" value="<?php echo $data["no_rekam_medis"];?>" <?=($data['no_rekam_medis'])?"readonly='readonly'":""?> />
                </div><!-- end form group -->
            </div><!-- end col-->
		</div><!-- end row -->
    </div><!--End Of Col-->	
		...
		<div class="row hidden">
			<div class="col-sm-12">
				<div class="form-group">
					...
					<br />
					<input type="radio" value="1" name="jns_pendaftar" required <?=($data['jns_pendaftar']==1)?"checked":""?> />&nbsp;Mendaftarkan Diri Sendiri
					<br />
					<input type="radio" value="2" name="jns_pendaftar" required <?=(!$data['jns_pendaftar'] or $data['jns_pendaftar']==2)?"checked":""?> />&nbsp;Mendaftarkan Orang Lain
				</div>
			</div>
		</div>
	</div>
...
<div class="row">
	...
	<div class="row">
		<div class="col-md-4">
			<div class="form-group">
				<label>Identitas<span class="asterix">&nbsp;*</span></label>
				<input name="jenis_identitas" value="<?=($data["jenis_identitas"])? $data["jenis_identitas"]:"KTP"?>" id="jenis_identitas" class="form-control required" type="text" readonly> 
			</div>
		</div>
		<div class="col-md-8">
			<div class="form-group">
				<label>NIK<span class="asterix">&nbsp;*</span></label>
				<input name="nik" value="<?=$data["nik"]?>" id="nik" class="form-control required" type="text" <?=($data['nik'])?"readonly='readonly'":""?> >
				<div class="nik_check_warning" style="color:#FF0000"></div>
			</div>
		</div>	
	...
	<div class="row">
		<div class="col-md-4">
			<div class="form-group">
				<label>Tanda Pengenal Lain</label>
				<select name="jenis_identitas_lain" id="jenis_identitas_lain" class="form-control">
					<option value=""></option>
		...
		<div class="col-md-8">
			<div class="form-group">
				<label>Nomor</label>
				<input name="no_identitas" value="<?=$data["no_identitas"]?>" id="no_identitas" class="form-control " type="text">            </div>
		</div>
	</div>
</div><!-- end col-->
</div><!-- endrow-->
...
<div class="row">
	...
	<div class="col-md-6">
		<div class="form-group"><span id="display_umur" class="pull-right text-muted"></span>
			<label for="nama">Tgl Lahir </label>
				...
				<div class="row">
					<div class="col-md-4">
						<select class="form-control required" name="tanggal_lahir" id="tanggal_lahir">
							<? for($i=1;$i<=31;$i++){?>
								<option<?=($tanggal_lahir==$i)? "selected":""?> value="<?=$i?>"><?=$i?></option>
							<? }?>
						</select>
					</div>
                    <div class="col-md-4">
                    ...
                    <input class="required" name="umur" id="umur" type="hidden" value="<?php echo $data["umur"];?>" />
				</div><!-- end row-->
			</div><!-- end form group -->
		</div><!-- end col -->
	</div><!-- end row-->
	...
	<div class="form-group">
	 ...
	 <div class="row">
		...
		<div class="col-md-6">
			<div class="form-group">
				<label for="jenis_pendidikan">Negara</label>
				<input type="text" name="negara" class="form-control" id="negara" value="<?=$data["negara"]?>">                  
			</div><!-- end form group-->
		</div><!-- end col-->
	</div><!-- end row-->
	...
	<div class="row">
		<div class="col-md-6">
			<div class="form-group">
				<label for="jenis_kelamin">No Telephone</label>
				 <input name="no_telp" id="no_telp" class="form-control required" value="<?=$data["no_telp"]?>" type="text">        	
			</div><!-- end form group -->
		</div><!-- end col-->
	
		<div class="col-md-6">
			<div class="form-group">
				<label for="golongan_darah">Handphone</label>
				<input name="no_hp" id="no_hp" class="form-control required" value="<?=$data["no_hp"]?>" type="text"> 
			</div><!-- /control-group category-->
		</div><!-- end col -->
	</div><!-- end row-->
	...
	<div class="form-group">
	 ...
	 <div class="row">
	   ...
	   <!-- <div class="row">
		...
		<div class="form-group">
			<label for="first_name">Nama Ayah:<span class="asterix">&nbsp;*</span></label>                    	 	
			<input name="ayah" value="<?=$data["ayah"]?>" id="ayah" class="form-control required" type="text">   
		</div>

		<div class="form-group">
			<label for="first_name">Nama Ibu:<span class="asterix">&nbsp;*</span></label>                    
			<input name="ibu" value="<?=$data["ibu"]?>" id="ibu" class="form-control required" type="text">   
		</div>
		
		<div class="form-group">
			<label>Nama Keluarga yang bisa dihubungi:<span class="asterix">&nbsp;*</span></label>
			<input name="klg_nama" value="<?=$data["klg_nama"]?>" id="klg_nama" class="form-control required" type="text">
		</div>
		
		 <div class="form-group">
			<label for="hubungan_keluarga">Hubungan Keluarga yang bisa dihubungi:</label>
			<?php $lookup_keluarga=$lookup_empty+$this->data_lookup["hubungan_keluarga"]; ?>
			<? echo form_dropdown("klg_hubungan",$lookup_keluarga,$data["klg_hubungan"],"id='klg_hubungan' class='form-control select2 required'");?>
		</div>
		
		<div class="form-group">
			<label for="alamat">Alamat Keluarga yang bisa dihubungi:<span class="asterix">&nbsp;*</span></label>                            
			<textarea name="klg_alamat" class="form-control required" rows="3"><?=$data["klg_alamat"]?></textarea>
		</div>                                
...
$(document).ready(function(){
	$('#no_rekam_medis').on("blur",function(){
		
		var daftar_no_rekam_medis 	=	[<?=$daftar_no_rekam_medis?>];	
		var no_rekam_medis			=	$(this).val();
	
		for(var i=0; i<daftar_no_rekam_medis.length; i++){
			if(no_rekam_medis==daftar_no_rekam_medis[i]){
				$(this).val("");
				sweetAlert("Maaf!", "No Rekam Medis "+no_rekam_medis+" Sudah Digunakan", "error");
			}
		}
	});
});
...
$(document).ready(function(){
	$('#nik').on("blur",function(){
		...
		var daftar_nik	=	[<?=$daftar_nik?>];	
		var nik			=	$(this).val();
	
		for(var i=0; i<daftar_nik.length; i++){
			if(nik==daftar_nik[i]){
				$(this).val("");
				sweetAlert("Maaf!", "NIK "+nik+" Sudah Terdaftar Sebelumnya", "error");
			}
		}
	});
});
...
<script>
	...
	function hitung_umur(){
        ...
        var today = new Date();
        var age = Math.floor((today-dob) / (365.25 * 24 * 60 * 60 * 1000))||0;
        $('#umur').val(age);	
        $('#display_umur').text("Usia: "+age+" thn");	
	}
</script>