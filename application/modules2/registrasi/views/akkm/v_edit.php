...
<?php
	...
	//$lookup_status_akkm["Private"]="Private";		
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <table class="table table-bordered table-condensed small-font">
							<tr>
								<td><b>Peta Geojson Uploaded</b></td>
								<td align="center" width="40">#</td>
							</tr>
							<? if(($data['the_geom']) && ($data['file_peta'])):?>
							
							<tr id="tr_<?=$data['idx']?>">
								<td>
									<? 
										$path = $this->config->item("dir_geojson") ;

									?>
									<a href="<?=$path.$data["file_peta"]?>" target="_blank">
									<i style="color:green" class="fa fa-file-archive-o" aria-hidden="true"></i>
									<?=" ".$data["file_peta"]?>
								</td>
								<td align="center">
							
								<a  style= "cursor: pointer; " class="del_geojson" data-idx="<?=$data['idx']?>">
                        ...
                        <div class="row">
							<div class="col-md-6">
                            	<div class="form-group">
									...
									<?php echo form_input('longitude',$data["longitude"],'id="x" placeholder="koordinat -90 sampai 90" data-x="'.$data["longitude"].'" class="form-control "');?>
                        	...
                        	<div class="col-md-6">
                            	<div class="form-group">
									...
									<?php echo form_input('latitude',$data["latitude"],'id="y" placeholder="koordinat -180 sampai 180" data-y="'.$data["latitude"].'" class="form-control "');?>
...
$(document).ready(function(){
	var xluas = <?=$data['luas']?>;
	var init_luas		=	ReplaceNumberWithCommas(xluas);
...
<script>
$(document).ready(function(){
	$(".del_geojson").on("click",function(e){
		
		var	konfirmasi	=	confirm('Anda yakin akan menghapus lampiran ini?');
		
		if(konfirmasi==true){
			
			var	url		=	"<?=$this->module?>delete_geojson";
			...
			$.ajax({
				...
				success	:	function(result){
					if(result){
						...
						new PNotify({
                            title   :   '<h6 style="margin-top:-1px;">Pemberitahuan</h6>',
                            text    :   'File Geojson Berhasil Dihapus',
                            type	:	'success',
                            hide    :   true
                        });
					}else{
						new PNotify({
                            title   :   '<h6 style="margin-top:-1px;">Pemberitahuan</h6>',
                            text    :   'File Geojson Gagal Dihapus',
                            hide    :   true
                        });
					}
				}
