...
class jenis extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->parent_module_title="Peta Tematik";
		$this->module_title="";
		$this->tbl_idx="idx";
		$this->tbl_sort="order_num asc";	
		
		//$this->lookup_map_group	=	lookup("map_layer_group","kd_group","ur_group",false,"order by order_num");
		$this->lookup_map_group	=	lookup("map_layer_group","kd_group","ur_group",false/*"kd_group not in ('TORA','PIAPS','HA')"*/,"order by order_num");
		//debug();
		//
		//exit();
		//debug();
	 ...
	 function listview(){
		...
		//$where[]="(category=1)";
        $where[]="(kd_group not in ('TORA','PIAPS','HA'))";
	...
	function mapfile() {debug();
		...
		$tplx='<!-- MapServer Template -->
				...
				<div>
				<link rel="stylesheet" href="https://tanahkita.id/assets/themes/flat/bootstrap/css/bootstrap.css">
				<style>
				.table {
    width: 100%;
    max-width: 100%;
    ...
    font-size: small;
		...
		$map='MAP
				...
				<table class="table table-condensed">
				  <tr style="background:#fafafa"><th colspan="3">'.$v['ur_layer'].'</th></tr>
	 ...
	 function get_layer_dbf($archive,$array=false) {
	 	for ($i = 0; $i < $archive->numFiles; $i++) {
			 if(strstr($archive->getNameIndex($i),".dbf")) $dbf_name=  $archive->getNameIndex($i);
		 }
		 
		 if ($dbf_name) {
			 $dbf_name=$this->config->item("dir_shapefile").$dbf_name;
			return ($array)?$this->echo_dbf($dbf_name):implode(",",$this->echo_dbf($dbf_name)); 
		 }
	 }		
	 
	 function add() {error_reporting(E_ALL & ~E_NOTICE & ~E_STRICT & ~E_DEPRECATED);
			...
			if ($_POST['filename']) {
				...
				if (file_exists($tmp_name)) {
					...
					if ($zip->open($tmp_name) === TRUE) {
						...
						$layer_dbf = $this->get_layer_dbf($zip);//get layer_name (dbf file in zip) used for MAPFILE LAYER TEMPLATE
						$zip->close();
						//unlink($tmp_name);
						if ($layer) {
				 		...
				 		//pre($this->config->item("dir_shapefile"));pre($layer);pre($layer_dbf);pre($data);echo $new_name;exit; 
					}
					if ($archive) {
						//exit;
