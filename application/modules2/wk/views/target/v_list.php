    ...
    <div class="row">
    	<div class="col-md-12">
        	...
        	<div class="box box-widget">
            	<div class="box-header with-borders">
                	<div class="row">
                        ...
                        <form class="form-inline" action="<?=$this->module?>" method="get">
                          ...
                          <div class="form-group">
							<?=form_dropdown("kd_group",array(""=>"Semua Wilayah Kelola")+$this->lookup_map_group,$_GET['kd_group'],"class='form-control'")?>
                          </div>
                          <div class="form-group">
                ...
                <table class="table table-bordered table-condensed small-font table-sort">
                    <thead>
                    	<tr>
                        <th width="20">No.</th>
                        <th class="forder" width="100" rel="date">Group</th>
                ...
                <tbody>
                    <?php if(cek_array($arrData)):?>
                            <?php foreach($arrData as $x=>$val):
									...
									$url_data = $this->module_data."index/".$val['kd_layer'];

									$url_edit = $this->module."edit/".$id;
									$url_delete = $this->module."delete/".$id;
		
									$status_badges = ($val['status'])?'<span class="label label-info">Published</span>':'<span class="label label-warning">Draft</span>';
							?>
                            	<tr id="tr_<?=$val['idx']?>">
                                	<td><?php echo $offset+$x+1?>.</td>
                                    <td rel="date_col" width="150"><?=$this->lookup_map_group[$val['kd_group']];?></td>
                    				<td rel="title_col"><a href="<?=$url_edit;?>"><?=$val['ur_layer'];?></a></td>
                    				<td><?=$val['description'];?></td>
                    				<td><?=$val['layer_data'];?></td>
                                    <td align="center">
										<span class="badge" style="height:16px;width:16px;background-color:<?=$val['layer_color'];?>;border:2px solid <?=$val['layer_outline_color'];?>">
										</span>
									</td>
                                    <td><?=$status_badges?></td>
	...
	$(function(){
		...
		$(".table-sort tbody").sortable({
			...
			update: function( event, ui ) {
				var nOrder = $(this).sortable('toArray').toString();
				//alert(module+'reorder'+nOrder);
				$.get(module+'reorder', {order:nOrder,prefix:'tr_'}, function( data ) {
