...
class potensi extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->parent_module_title="Potensi";
		$this->module_title="";
		$this->tbl_idx="idx";
	 ...
	 function init_lookup(){
		...
		$lookup_arr=$this->model_lookup->SearchRecordWhere("active is null","order by lookup_category,order_num");
	 ...
	 function add(){
        ...
        if($act=="create"):
			...
			if($_FILES['file_peta']['name']!=''){
				...
				if ($id_last) {
					$sql = "UPDATE t_daftar_wikera SET the_geom='".$json."', file_peta='".$datafile_peta."' WHERE idx='".$id_last."'";
