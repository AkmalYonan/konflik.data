...
class petax extends Admin_Controller {
	...
	function wmslayer2() {
		...
		$arrData=$this->conn->GetAll("select a.ur_group,a.kd_group as kd_group2,a.ur_group as ur_group2,b.kd_layer,b.ur_layer from map_layers b left join map_layer_group a on a.kd_group=b.kd_group where a.kd_group not in ('TORA','PIAPS','HA') and b.status=1 order by b.order_num");
		...
		$ret .= "layerControlWms = L.control.groupedLayers(null, null,{label:'LAYER/PETA LAIN',fa:'map-o fa-lg',groupCheckboxes: true});\n";
		$ret .= "wmsLayers=".(json_encode($arrData)).";\n";
		$ret .= "for(i=0;i<wmsLayers.length;i++) {\n";
		$ret .= "	layerControlWms.addOverlay(source.getLayer(wmsLayers[i].kd_layer), wmsLayers[i].ur_layer,wmsLayers[i].ur_group);\n";
		$ret .= "}\n";
