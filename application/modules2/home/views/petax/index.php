...
<script>
	...
	var basemaps = [
		...
		L.tileLayer('//stamen-tiles-{s}.a.ssl.fastly.net/toner-lite/{z}/{x}/{y}.png', {
			attribution: 'Map tiles by <a href="https://stamen.com">Stamen Design</a>, <a href="http://creativecommons.org/licenses/by/3.0">CC BY 3.0</a> &mdash; Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors, <a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>',
			subdomains: 'abcd',
			maxZoom: 20,
			minZoom: 0,
			label: 'Toner Lite'
	...
	function set_wk_layer(){
		...
		for(var i=0;i<wk_group.length;i++) {
			...
			layer.push({"ur_group":wk_group[i].title+" <span id='"+wk_group[i].id+"'></span>","json_url":wk_group[i].url+"&piaps=1","ur_layer":"PS","style":wk_group[i].style});
			layer.push({"ur_group":wk_group[i].title+" <span id='"+wk_group[i].id+"'></span>","json_url":wk_group[i].url+"&hutan_adat=1","ur_layer":"WILAYAH ADAT","style":wk_group[i].style});
			layer2.push({"ur_layer":"dummy"});
			layer2.push({"ur_layer":"dummy"});
			layer2.push({"ur_layer":"dummy"});
		}
		for(var i=0;i<akkm_group.length;i++) {
			layer.push({"ur_group":akkm_group[i].title+" <span id='"+akkm_group[i].id+"'></span>","json_url":akkm_group[i].url,"ur_layer":"AKKM","style":akkm_group[i].style});
			layer2.push({"ur_group":akkm_group[i].title+" <span id='"+akkm_group[i].id+"'></span>","json_url":akkm_group[i].url+"&z=11","ur_layer":"AKKM","style":akkm_group[i].style});
		}
		
		
		//var tora_piaps_url = base_url+"data_service/map/tora_piaps.geojson?limit=-1&x_gte=95&y_gte=-11&y_lte=11";
		//var tora_piaps_url_usulan = base_url+"data_service/map/tora_piaps_usulan.geojson?limit=-1&x_gte=95&y_gte=-11&y_lte=11";
		//var tora_piaps_url_realisasi = base_url+"data_service/map/tora_piaps_realisasi.geojson?limit=-1&x_gte=95&y_gte=-11&y_lte=11";
		...
		/*
		$.each(layer,function(i,dat){
			$.getJSON(dat.json_url,function(data,status){
				...
				layerControl.addOverlay(lyr, dat.ur_layer,dat.ur_group);
		...
		*/
		...
		for(var i=0;i<layer.length;i++) {
			...
			(function(i) {
				...
				if (layer[i].ur_layer==layer2[i].ur_layer) {
					$.getJSON(layer2[i].json_url,function(data,status){
						lyr2 = L.geoJson(data,{style :layer2[i].style,onEachFeature : onEachFeatureTora,pointToLayer: function (feature, latlng) {
							return L.circleMarker(latlng, layer2[i].style);
						}});
					}); 
					map.on('zoomend ', function(e) {
						 if ( map.getZoom() >9 ){ map.removeLayer( lyr );map.addLayer( lyr2 )}
						 else if ( map.getZoom() <= 9 ){ map.addLayer( lyr );map.removeLayer( lyr2 );}
					});
				}
			})(i);
		}
		...
		*/
		/*var waOverlays={
			"WILAYAH ADAT":{"Pengakuan Wil. Adat":Wa_Pengakuan}
		};
		layerControlWa = L.control.groupedLayers(null, waOverlays);
		layerControlWa.addTo(map);*/
	}
	...
	function onEachFeatureTora(feature, layer) {
		if (feature.properties){
			...
			if (jenis_wilayah) {
				...
				layer.bindTooltip("<strong>"+jenis_wilayah+"</strong><BR>Nama Wilayah: "+arr_data.nama_wikera,{permanent: false});
...
<style>
	...
	<div id="map-wrapper" style="position:relative">
    		...
    		<div class="row hidden" style="position:relative; padding-top:10px;">
                       <div class="col-sm-6">
                            <h6  style="padding-left:15px;ext-transform:uppercase">PETA SEBARAN KONFLIK & WILAYAH KELOLA</h6>
                        </div>
                        <div class="col-sm-6">
