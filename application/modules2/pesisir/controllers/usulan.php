...
class usulan extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->parent_module_title="Usulan Masyarakat";
		$this->module_title="";
		$this->tbl_idx="idx";
	 ...
	 function index(){
		...
		$where[]="( kode_tahapan not in('T7','T8','T0'))";
	  ...
	  public function get_export_template(){
		...
		// endif;
		$filter_table[]	=	"( kode_tahapan not in('T7','T8','T0'))";
	 ...
	 function add(){
        ...
        if($act=="create"):
			...
			for ($i=0; $i<count($data['jenis_surat']); $i++){
				...
				if (($_FILES["lampiran"]["size"][$i] < $maxSize) && in_array($file_extension, $validextensions)){
					if (move_uploaded_file($_FILES['lampiran']['tmp_name'][$i], $fix_path)) {//if file moved to uploads folder
						...
						$dataInsert["jenis_surat"] =$data['jenis_surat'][$i];
			...
			for ($i=0; $i<count($data['jenis_aturan']); $i++){
				$maxSize="5000000";
				$target_path = $this->config->item("dir_lampiran_wikera"); //Declaring Path for uploaded images
				$validextensions = array("jpeg", "jpg", "JPEG", "JPG","doc","docx","xls","xlsx","pdf","PDF"); 
				$ext = explode('.', basename($_FILES['lampiran']['name'][$i]));
				...
				if (($_FILES["lampiran"]["size"][$i] < $maxSize) && in_array($file_extension, $validextensions)){
					if (move_uploaded_file($_FILES['lampiran']['tmp_name'][$i], $fix_path)) {//if file moved to uploads folder
						unset($dataInsert);
						$dataInsert["idx_parent"]	=	$id_last;
						$dataInsert["jenis_aturan"] =$data['jenis_aturan'][$i];
						if($_FILES['lampiran']['name'][$i]){
							$dataInsert['lampiran']	=	date('s').time().str_replace(' ', '', $_FILES['lampiran']['name'][$i]) . "." . $ext[count($ext) - 1];
						}
						$this->model_aturan->InsertData($dataInsert);
					} else {//if file was not moved.
						$dataInsert["idx_parent"]	=	$id_last;
						$dataInsert["jenis_aturan"] =$data['jenis_aturan'][$i];
						$this->model_aturan->InsertData($dataInsert);
	...
	function edit($id){
        ...
        if(empty($act)):
			...
			$arrAturan		=	$this->model_aturan->ListAll("idx_parent='".$id."'");
		...
		if($act=="update"):
			...
			for ($i=0; $i<count($data['jenis_aturan']); $i++){
				...
				$ext = explode('.', basename($_FILES['lampiran']['name'][$i]));
				$file_extension = end($ext);
				$fix_path = "/uploads/" . date('s').time().str_replace(' ', '', $_FILES['lampiran']['name'][$i]) . "." . $ext[count($ext) - 1];//set the target path with a new name of image
				if (($_FILES["lampiran"]["size"][$i] < $maxSize) && in_array($file_extension, $validextensions)){
	...
	public function delete_aturan(){
		...
		$arrData	=	$this->model_aturan->GetRecordData("idx='".$idx."'");
		
		if(cek_array($arrData['lampiran'])):
			unlink($this->config->item("dir_lampiran_wikera").$arrData['lampiran']);
		endif;
		
		$delete		=	$this->model_aturan->DeleteData("idx='".$idx."'");
	...
	public function ubah_aturan(){
		...
		$start		=	$this->conn->StartTrans();
			...
			$this->model_aturan->UpdateData($data,$criteria);
	...
	public function ubah_lampiran_aturan(){
		...
		if (($_FILES["file"]["size"] < $maxSize) && in_array($file_extension, $validextensions)){
			if (move_uploaded_file($_FILES['file']['tmp_name'], $fix_path)) {//if file moved to uploads folder
				...
				$this->model_aturan->UpdateData($dataUpdate,$criteria);
	...
	function ExportCSV(){
		...
		$result			=	$this->pppbm_model->export_csv($filter,$limit,$offset);
