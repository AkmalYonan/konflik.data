                ...
                <form id="frm" method="post" action="<?php echo $this->module;?>edit" enctype="multipart/form-data">
                  ...
                  <div class="row">
                    <div class="col-md-6">
									...
									<select name="kategori_pppbm" id="kategori_pppbm" class="form-control">
										<?php if(cek_array($arr_pppbm)): ?>
											...
											<?php foreach($arr_pppbm as $k=>$v): ?>
												<option value="<?=$k?>" <?=($k==$data['kategori_pppbm'])?"selected":""?>><?=$v?></option>
											<?php endforeach; ?>
										<?php endif; ?>
									</select>
								</div>
									...
									<select name="kode_jenis_hak" class="form-control">
										...
										<?php if(cek_array($arr_jns_hak)): ?>
											<?php foreach($arr_jns_hak as $k=>$v): ?>
												<option value="<?=$k?>"<?=($k==$data['kode_jenis_hak'])?"selected":""?>><?=$v?></option>
											<?php endforeach; ?>
										<?php endif; ?>
									</select>
								</div>
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									...
									<div class="input-group">
										<input class="form-control" name='luas_larang' type="text" name="my_field" id="my_field" pattern="^\d*(\.\d{0,2})?$" value="<?php echo $data["luas_larang"];?>"  />
						...
						<?php
						$checkbox_array = explode(", ", $data['aturan_pengelolaan']);
						// pre($checkbox_array);
						?>
						<div class="row">
							<div class="col-md-7">
								...
								<div class="form-group">
										<div class="col-md-6">
									<div class="input-group">
										<input type="checkbox" id="aturan_pengelolaan" name="aturan_pengelolaan[]" value="Tidak ada" <?=(in_array("Tidak ada", $checkbox_array))?"checked":""?>> Tidak ada<br>
										<input type="checkbox" id="aturan_pengelolaan" name="aturan_pengelolaan[]" value="Ada aturan waktu" <?=(in_array("Ada aturan waktu", $checkbox_array))?"checked":""?>> Ada aturan waktu<br>
										<input type="checkbox" id="aturan_pengelolaan" name="aturan_pengelolaan[]" value="Ada aturan alat tangkap" <?=(in_array("Ada aturan alat tangkap", $checkbox_array))?"checked":""?>> Ada aturan alat tangkap<br>
										<input type="checkbox" id="aturan_pengelolaan" name="aturan_pengelolaan[]" value="Ada aturan konservasi" <?=(in_array("Ada aturan konservasi", $checkbox_array))?"checked":""?>> Ada aturan konservasi<br>
									</div>
									</div>
									<div class="col-md-6">
										<div class="input-group">
										<input type="checkbox" id="aturan_pengelolaan" name="aturan_pengelolaan[]" value="Ada aturan jenis" <?=(in_array("Ada aturan jenis", $checkbox_array))?"checked":""?>> Ada aturan jenis<br>
										<input type="checkbox" id="aturan_pengelolaan" name="aturan_pengelolaan[]" value="Ada aturan wilayah" <?=(in_array("Ada aturan wilayah", $checkbox_array))?"checked":""?>> Ada aturan wilayah<br>
										<input type="checkbox" id="aturan_pengelolaan" name="aturan_pengelolaan[]" value="Ada aturan ukuran" <?=(in_array("Ada aturan ukuran", $checkbox_array))?"checked":""?>> Ada aturan ukuran<br>
										<input type="checkbox" id="aturan_pengelolaan" name="aturan_pengelolaan[]" value="Ada aturan habitat" <?=(in_array("Ada aturan habitat", $checkbox_array))?"checked":""?>> Ada aturan habitat<br>
										</div>
									</div>
								</div>
							</div>
					...
					<div class="col-md-6">
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									<label>Lampiran file Peta</label> <?php echo ($data['file_peta']?"<small>(".$data['file_peta'].")</small>":"<span class='label label-danger'>Belum ada</span>")?></small>
									<input id="imgInpPlay" title="Klik untuk menambahkan/mengganti file peta" type="file" name="file_peta" class="form-control" style="padding:0" />
						...
						<?php if (!$data['geo']):?><div class="alert alert-danger">Belum ada peta GeoJSON</div><?php endif?>
						<div class="row">
							<div class="col-sm-12">
								<div class="callout callout-info" style="margin-bottom:-10px;">
									<small align="justify">File Peta Spasial (Format: GeoJSON), dengan Projection EPSG:4326 dan Ukuran File Maks: 1.2M</small>
								</div>
							</div>
						</div>
					...
					<div class="col-md-12">
						...
						<div class="row">
							<div class="col-sm-12 table-responsive">
								<!-- <?=pre($aturan);?> -->
								<?php if(cek_array($aturan)): ?>
									<strong>Aturan Tersimpan</strong>
									<table class="table table-condensed table-striped">
										<?php foreach($aturan as $k=>$v): ?>
											<tr id="aturan_<?=$v['idx']?>">
												<td id="data_aturan_<?=$v['idx']?>"><?=$v['jenis_aturan']?></td>
												<td id="lampiran_aturan_<?=$v['idx']?>">
												...
												<td width="150">
													<a class="btn btn-success btn-xs edit_aturan" data-idx="<?=$v['idx']?>">
														<i class="fa fa-pencil">&nbsp;</i>Edit
													</a>
													<a class="btn btn-danger btn-xs del_aturan" data-idx="<?=$v['idx']?>">
											...
											<div class="row">
												<!-- <div class="col-md-11" style="margin-bottom:2px;">
													<div style="position:relative">
														<div class="row">
															<div class="col-sm-6">
																<select id="valid" name="jenis_aturan[]" class="form-control">
						...
						<div class="row">
							<div class="col-sm-12 table-responsive">
								<?php if(cek_array($perda)): ?>
									<strong>Surat Keputusan Tersimpan</strong>
									<table class="table table-condensed table-striped">
										<?php foreach($perda as $k=>$v): ?>
											<tr id="sk_<?=$v['idx']?>">
												<td id="data_sk_<?=$v['idx']?>"><?=$v['jenis_surat']?> No.<?=$v['nomor']?> Tentang <?=$v['tentang']?> Tahun <?=$v['tahun']?></td>
									...
									<div class="row">
										<!-- <div class="col-md-11" style="margin-bottom:2px;">
											<div style="position:relative">
												<div class="row">
													<div class="col-sm-3">
														<select id="valid" name="jenis_surat[]" class="form-control">
										...
										<span class="btn btn-sm du-remove" style="margin-left:30px; top:0px"><i class="fa fa-trash"></i></span> -->
									</div>
								</div>
							</div>
						</div>
									...
									<select name="Keterlibatan_perempuan" id="Keterlibatan_perempuan" class="form-control">
										<?php if(cek_array($arr_prmpuan)): ?>
											...
											<?php foreach($arr_prmpuan as $k=>$v): ?>
												<option value="<?=$k?>" <?=($k==$data['Keterlibatan_perempuan'])?"selected":""?>><?=$v?></option>
											<?php endforeach; ?>
										<?php endif; ?>
									</select>
								</div>
...
<div id="aturanModal" class="modal fade" role="dialog">
	<div class="modal-dialog">
		...
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="modal-title"><i class="fa fa-pencil">&nbsp;</i>Ubah Data Aturan Perikanan dan Pengelolaan Pesisir</h4>
			</div>
			<div class="modal-body">
				<div class="row">
					<div class="col-sm-12">
						<div class="form-group">
							...
							<input type="hidden" id="id_aturan" class="form-control" />
							<select id="ubah_jenis_aturan" class="form-control">
							<option value="">Pilih Jenis Aturan</option>
							<option value="Peraturan adat">Peraturan adat</option>
							<option value="Peraturan kelompok">Peraturan kelompok</option>
							<option value="Peraturan desa">Peraturan desa</option>
					...
					<div class="col-sm-12">
						<div class="form-group">
							<label>Lampiran</label>
							<input type="file" id="ubah_lampiran_aturan" />
						</div>
					</div>
				</div>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-primary ubah_aturan_submit">Simpan</button>
				<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
			</div>
		</div>
...
$(document).ready(function(){
	...
	$("#ubah_lampiran_aturan").on("change",function(){
		
		var url			=	"<?=$this->module?>ubah_lampiran_aturan";
		var file_data	=	$('#ubah_lampiran_aturan').prop('files')[0];
        var form_data	=	new FormData();
		var idx			=	$("#id_aturan").val();
		form_data.append('idx', idx);
		form_data.append('file', file_data);
		...
		$.ajax({
			...
			success		:	function(result){
				...
				if(result.hasil==1){
					...
					$("#lampiran_aturan_"+idx).html(result.data);
...
<script>
$(document).ready(function(){
	$(".edit_aturan").on("click",function(){
		
		var	idx	=	$(this).data("idx");
		var	url	=	"<?=$this->module?>data_aturan";
		...
		$.ajax({
				...
				success	:	function(result){
					...
					var id_aturan			=	result.idx;
					var jenis_aturan		=	result.jenis_aturan;
					
					$("#id_aturan").val(id_aturan);
					$("#ubah_jenis_aturan").val(jenis_aturan);
					
					$("#aturanModal").modal("show");
				}
		
		});
	});
	
	$(".ubah_aturan_submit").on("click",function(){
		
		var	idx				=	$("#id_aturan").val();
		var jenis_aturan			=	$("#ubah_jenis_aturan").val();
		
		var	url				=	"<?=$this->module?>ubah_aturan";
		...
		$.ajax({
				...
				type	:	"POST",
				dataType:	"JSON",
				data	:	{idx:idx,jenis_aturan:jenis_aturan},
				success	:	function(result){
					
					if(result){
						
						$("#data_aturan_"+idx).text(jenis_aturan);
						
						$("#aturanModal").modal("hide");
						new PNotify({
                            title   :   '<h6 style="margin-top:-1px;">Pemberitahuan</h6>',
                            text    :   'Ubah Surat Keputusan Berhasil',
                            type	:	'success',
...
<script>
$(document).ready(function(){
	$(".del_aturan").on("click",function(e){
		var	konfirmasi	=	confirm('Anda yakin akan menghapus data ini?');
		
		if(konfirmasi==true){
			
			var	url		=	"<?=$this->module?>delete_aturan";
			var idx		=	$(this).data("idx");
			...
			$.ajax({
				...
				success	:	function(result){
					if(result){
						...
						new PNotify({
                            title   :   '<h6 style="margin-top:-1px;">Pemberitahuan</h6>',
                            text    :   'Hapus Aturan Keputusan Gagal',
                            hide    :   true
                        });
					}
				}
