...
class data extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->parent_module_title="TORA/PIAPS";
		$this->module_title="<i class='fa fa-exclamatin'></i> Under Development ";
		$this->tbl_idx="idx";
	 ...
	 function add(){
        ...
        if($act=="create"):
			...
			$data["kd_layer"]=$data["kd_group"];
			$data["status"]=$data["status"]?1:0;

			
			$json=file_get_contents($uploaded_file['full_path']);pre($json);
			...
			$idx = $this->model->GetLastID();pre($idx);
			if ($idx) {
				$sql = "UPDATE map_data_detail SET the_geom=ST_GeomFromGeoJSON('".$json."') WHERE idx='".$idx."'";
