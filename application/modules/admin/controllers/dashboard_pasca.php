...
class dashboard_pasca extends Admin_Controller {
	...
	function index(){ 
		...
		$filter = "(".$sql_bulan." and year(tgl_status_proses)='".$tahun."') and  status_rehab='3' and status_program<>'DO'";
		...
		if ($kd_inst) :
			if($kd_inst!="BNNP" && $kd_inst!="BNNK"):
				...
				if ($kd_org) {
					$filter.=" and (rujuk_pasca = '".$kd_org."' or rujuk_lanjut = '".$kd_org."')";
					...
					$filter.=" and (rujuk_pasca REGEXP 'bb|bl' or rujuk_pasca REGEXP 'bb|bl')";
					$this->lookup_blbb	=	$this->conn->GetAll("select kd_instansi,nama_instansi,x,y from m_instansi where (jenis_instansi='bl' or jenis_instansi='bb')");
				}
			...
			if($kd_inst=="BNNP"):	
				...
				if ($kd_org) {
					$substr_1=1;
					$substr_2=4;
					$filter.=" and substr(kd_wilayah,".$substr_1.",".$substr_2.") like '".$kd_inst_wil[0]."%' and (rujuk_pasca REGEXP 'BNNP|BNNK|RD|KM' or rujuk_lanjut REGEXP 'BNNP|BNNK|RD|KM')";	
					...
					$filter.=" and (rujuk_pasca REGEXP 'BNNP|BNNK|RD|KM' or rujuk_lanjut REGEXP 'BNNP|BNNK|RD|KM')";	
					$data['jsmap']='00';
					$level=1;
				}
			endif;
			if($kd_inst=="BNNK"):	
				...
				$substr_1=1;
				$substr_2=4;
				$filter.=" and substr(kd_wilayah,".$substr_1.",".$substr_2.") like '".$kd_inst_wil[0].$kd_inst_wil[1]."%' and (rujuk_pasca REGEXP 'BNNP|BNNK|RD|KM' or rujuk_lanjut REGEXP 'BNNP|BNNK|RD|KM')";	
				$data['jsmap']=$kd_inst_wil[0];
		...
		$sql_pasien = "
			select 
				...
				sum(CASE WHEN (status_program='PS') THEN 1 else 0 end) as pasca_proses,
				sum(CASE WHEN (status_program='SL' and outcome_pasien is not null) THEN 1 else 0 end) as pasca_selesai,


				sum(CASE WHEN status_program='MD' THEN 1 else 0 end) as sp_md,
				sum(CASE WHEN status_program='PS' and status_proses='PRAP' THEN 1 else 0 end) as prap,
				sum(CASE WHEN status_program='PS' and status_proses='PRRIDA' THEN 1 else 0 end) as da,
				sum(CASE WHEN status_program='PS' and status_proses='PRRIDR' THEN 1 else 0 end) as dr,
				
				sum(CASE WHEN status_program='PS' and status_proses='PRRJPG' THEN 1 else 0 end) as pg,
				sum(CASE WHEN status_program='PS' and status_proses='PRRIPD' THEN 1 else 0 end) as pd,
				sum(CASE WHEN status_program='PS' and status_proses='PRRIDK' THEN 1 else 0 end) as dk,
				
				sum(CASE WHEN status_program='PS' and status_proses='PRRLPUKP' THEN 1 else 0 end) as pukp,
				sum(CASE WHEN status_program='PS' and status_proses='PRRLPUEP' THEN 1 else 0 end) as puep,
				sum(CASE WHEN status_program='PS' and status_proses='PRRLPUTU' THEN 1 else 0 end) as putu,
				
				sum(CASE WHEN status_program='PS' and status_proses='PRRLPDHV' THEN 1 else 0 end) as pdhv,
				sum(CASE WHEN status_program='PS' and status_proses='PRRLPDPK' THEN 1 else 0 end) as pdpk,
				sum(CASE WHEN status_program='PS' and status_proses='PRRLPDKN' THEN 1 else 0 end) as pdkl,
				sum(CASE WHEN status_program='PS' and status_proses='PRRLPDTU' THEN 1 else 0 end) as pdtu,
				
				sum(CASE WHEN (status_program='SL' and outcome_pasien='PP') THEN 1 else 0 end) as pulih_produktif,
				sum(CASE WHEN (status_program='SL' and outcome_pasien='PTP') THEN 1 else 0 end) as pulih_tidak_produktif,
				sum(CASE WHEN (status_program='SL' and outcome_pasien='TPP') THEN 1 else 0 end) as tidak_pulih_produktif,
				sum(CASE WHEN (status_program='SL' and outcome_pasien='TPTP') THEN 1 else 0 end) as tidak_pulih_tidak_produktif,
				count(idx) as jumlah 
		...
		//pre($data_pasien);\
		//debug(false);
		//$sql_wil = "select a.* from (select substring(kd_wilayah,".$substr_1.",".$substr_2.") as kd,count(idx) as jumlah from ".$table_rehab." where ".$filter." and ((inst_pasca='BNNP' or inst_pasca='BNNK') or (inst_lanjut='BNNP' or inst_lanjut='BNNK')) group by substring(kd_wilayah,".$substr_1.",".$substr_2.")) a order by jumlah desc";
		...
		//wil untuk assesment
		//wil untuk pasca
		$sql_wil_p = "select a.* from (select ".$as_kd.",".$status_x.",count(idx) as jumlah from ".$table_rehab." where ".$filter." and (status_rehab='3' and inst_lanjut is null and inst_pasca IN ('BNNP','BNNK','RD')) group by kd,kd2) a order by jumlah desc";
		$arrWilp=$this->conn->GetAll($sql_wil_p);
		//wil untuk pasca lanjut
		$sql_wil_s = "select a.* from (select ".$as_kd.",".$status_x.",count(idx) as jumlah from ".$table_rehab." where ".$filter." and (status_rehab='3' and inst_lanjut is not null and inst_lanjut IN ('BNNP','BNNK','RD')) group by kd,kd2) a order by jumlah desc";
		$arrWils=$this->conn->GetAll($sql_wil_s);
		$arrWilx=array_merge($arrWilp,$arrWils);
		...
		if(cek_array($arrWilx)):
			foreach($arrWilx as $x=>$val):
				...
				$data_jml_wil["id_pop"][$val["kd"]]["a"]+=$val["assesment"]*1;
				$data_jml_wil["id_pop"][$val["kd"]]["r"]+=$val["rehab"]*1;
				$data_jml_wil["id_pop"][$val["kd"]]["p"]+=$val["pasca"]*1;
				$data_jml_wil["id_pop"][$val["kd"]]["s"]+=$val["selesai"]*1;
		...
		$arrTrend=$this->conn->GetAll("select year(tgl_mulai_pasca) as tahun,count(idx) as jumlah from ".$table_rehab.$filter_trend." group by year(tgl_mulai_pasca)");
		...
		//TOTAL PER BULAN
		$arrJml=$this->conn->GetAll("select jenis_kelamin,month(tgl_mulai_pasca) as bulan,year(tgl_mulai_pasca) as tahun,count(idx) as jumlah from ".$table_rehab." where ".$filter." group by month(tgl_mulai_pasca),year(tgl_mulai_pasca),jenis_kelamin");
		if(cek_array($arrJml)):
	...
	function index2(){
		...
		if($kd_propinsi):
			...
			$kd="_wilayah";
			$data['jsmap']=substr($kd_propinsi,-2);
			$filter .= " and substring(rujuk_pasca,".$substr_1.",".$substr_2.") like '" .$kd_propinsi."%'";
		endif;
		$data['level']=$level;
