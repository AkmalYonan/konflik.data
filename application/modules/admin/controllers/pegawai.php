...
class pegawai extends Admin_Controller {
    ...
    function __construct(){
	 ...
	 function listview(){
		...
		if($this->input->get_post("posisi")):
            $where[]="posisi = '".$this->input->get_post("posisi")."'";
        endif;
        ...
        if($data['prov'] == '' && $data['kabupaten'] == '' && $data['skpd'] == ''):
			...
			$whereSql=" propinsi like '%".$data['prov']."%' AND kabupaten like '%".$data['kabupaten']."%' AND skpd = '".$data['skpd']."' ";	
			if(cek_array($where)):
	...
	function search_personel(){
	 	$x=$_GET['q'];
		$res=("select * from sppp_data_pegawai where nip like '%$x%' or nama like '%$x%' group by nip");
		$xs=$this->conn->GetAll($res);
		// debug();
		$countQuery = $this->conn->GetAll("select count(*) as mycount from ($res) a ");
        // pre($countQuery);exit;
		$data_count= $countQuery;
        $data_rows= $res;
         
        $datax["total_count"]=$data_count[0]['mycount'];
        $datax["items"]=$xs;
	...
	function view($id){
		...
		$data['getLog']=$this->pegawai_model->getLog($id,$fg='a');
        $data["data"]=$arrData;
		// pre($data['getLog']);exit;
	 ...
	 public function delete_berkas($id)
	{
		...
		$this->_proses_message($ok,$this->module."view/$idp",$this->module."view/$idp");
	...
	public function publishx()
	{
		...
		$ok=$this->conn->CompleteTrans();
		$this->_proses_message($ok,$this->module."view/$id",$this->module."view/$id");
	...
	function get_city($kodex) 
	{
		...
		$jsonData = file_get_contents($services_prov."kabupaten/$kode");
	...
	function update_cek(){
		...
		$arrData=$this->model_berkas->GetRecordData("idx=$id");
		if($arrData['cek']==1):
			$data["cek"]=0;
		else:
			$data["cek"]=1;
		endif;
        // $this->msg_ok="Data successfull verified";
        // $this->msg_fail="Data successfull not verified";
		// debug();
		$this->conn->StartTrans();
		$arr = $this->model_berkas->UpdateData($data,"idx=$id");
