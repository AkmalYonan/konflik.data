...
<script>
	...
	var pesisir = {
		"pesisir_group":[
		{
			'id':'potensi',
			'title':'Potensi',
			'url':	base_url+"data_service/map/pesisir_potensi.geojson?limit=-1&x_gte=95&y_gte=-11&y_lte=11",
			'style':styleTora,
		},
		{
			'id':'usulan',
			'title':'Usulan Masyarakat',
			'url':	base_url+"data_service/map/pesisir_usulan.geojson?limit=-1&x_gte=95&y_gte=-11&y_lte=11",
			'style':styleTora,
		},
		{
			'id':'realisasi',
			'title':'Realisasi',
			'url':	base_url+"data_service/map/pesisir_realisasi.geojson?limit=-1&x_gte=95&y_gte=-11&y_lte=11",
			'style':styleTora,
		}
		],
		"pesisir_legend":base_url+"data_service/map/pesisir_legend.html"
	};
	...
	function set_pesisir_layer(){
		var layer=[];var layer2=[];
		var pesisir_group = pesisir.pesisir_group;
		var pesisir_legend = pesisir.pesisir_legend;
		for (var i = 0; i < pesisir_group.length; i++) {
        layer.push({
            "ur_group": pesisir_group[i].title + " <span id='" + pesisir_group[i].id + "'></span>",
            "json_url": pesisir_group[i].url + "&tora=1",
            "ur_layer": "TORA",
            "style": pesisir_group[i].style
        });
        layer.push({
            "ur_group": pesisir_group[i].title + " <span id='" + pesisir_group[i].id + "'></span>",
            "json_url": pesisir_group[i].url + "&piaps=1",
            "ur_layer": "PERHUTANAN SOSIAL",
            "style": pesisir_group[i].style
        });
        layer.push({
            "ur_group": pesisir_group[i].title + " <span id='" + pesisir_group[i].id + "'></span>",
            "json_url": pesisir_group[i].url + "&hutan_adat=1",
            "ur_layer": "HUTAN ADAT",
            "style": pesisir_group[i].style
        });
        layer.push({
            "ur_group": pesisir_group[i].title + " <span id='" + pesisir_group[i].id + "'></span>",
            "json_url": pesisir_group[i].url + "&pppbm=1",
            "ur_layer": "PPPBM",
            "style": pesisir_group[i].style
        });
    	}		
		pesisirOverlays={"WILAYAH PESISIR":{"a":false}};
		pesisirOverlays={
			"WILAYAH PESISIR":{"x":false}
		};
		var options = {
		  label:'WILAYAH PESISIR',
		  fa:'anchor fa-lg',
		  exclusiveGroups: ["WILAYAH PESISIR"],
		...
		for(var i=0;i<layer.length;i++) {
			(function(i) {
				...
				$.getJSON(layer[i].json_url,function(data,status){
					plyr = L.geoJson(data,{style :layer[i].style,onEachFeature : onEachFeatureTora,pointToLayer: function (feature, latlng) {
						return L.circleMarker(latlng, layer[i].style);
					}});
					//pesisirOverlays['WILAYAH KELOLA'][layer[i].ur_layer] = 
					//lyrx[layer[i].ur_group][]=lyr;
					layerControl.addOverlay(plyr, layer[i].ur_layer,layer[i].ur_group);
					//$(data.features).each(function(key, data) {
					//	pesisirOverlays['WILAYAH KELOLA'][ur_layer].addData(data);
					//});
					map.addLayer( plyr );
				}); 
			})(i);
    ...
    for (var i = 0; i < wk_group.length; i++) {
        layer.push({
            "ur_group": wk_group[i].title + " <span id='" + wk_group[i].id + "'></span>",
            "json_url": wk_group[i].url + "&tora=1",
            "ur_layer": "TORA",
            "style": wk_group[i].style
        });
        layer.push({
            "ur_group": wk_group[i].title + " <span id='" + wk_group[i].id + "'></span>",
            "json_url": wk_group[i].url + "&piaps=1",
            "ur_layer": "PERHUTANAN SOSIAL",
            "style": wk_group[i].style
        });
        layer.push({
            "ur_group": wk_group[i].title + " <span id='" + wk_group[i].id + "'></span>",
            "json_url": wk_group[i].url + "&hutan_adat=1",
            "ur_layer": "HUTAN ADAT",
            "style": wk_group[i].style
        });
    }
    for (var i = 0; i < akkm_group.length; i++) {
        layer.push({
            "ur_group": akkm_group[i].title + " <span id='" + akkm_group[i].id + "'></span>",
            "json_url": akkm_group[i].url + "&z=11",
            "ur_layer": "AKKM",
            "style": akkm_group[i].style
        });
    }
    ...
    $(".a-layer-group").each(function (i, val) {
        ...
        $.each(layer_detail, function (i, val) {
            group.addLayer(source.getLayer(val));
        });

        layer_wms_group[layer_id] = group;
        layerControl.addOverlay(layer_wms_group[layer_id], layer_id, "Program Pemerintah");
		});
	...
	function onEachFeatureTora(feature, layer) {
		if (feature.properties){
			...
			if (jenis_wilayah) {
				...
				}else if(jenis_wilayah=="PPPBM"){
					nama_jenis_wilayah	=	"PPPBM";
				}else{
					nama_jenis_wilayah	=	"Hutan Adat";
				}
				if(nama_jenis_wilayah=="PPPBM"){
					layer.bindTooltip("<strong>"+nama_jenis_wilayah+"</strong><BR>Nama Wilayah Kelola: "+arr_data.nama_wikera,{permanent: false});
				layer.bindPopup("<div class='pop-tipe'>"+arr_data.nama_wikera+"</div><div class='row'><div class='col-sm-12' style='margin-top:5px;'>"+nama_jenis_wilayah+"</div></div><div class='row'><div class='col-sm-12'><p align='justify'>"+arr_data.short_narasi+"</p></div></div><div class='row'><div class='col-sm-12'><a href='data/pesisir/detil/"+feature.properties.id_enc+"'><button class='btn btn-primary btn-block btn-sm'>View Detail</button></a></div></div>")
				}
				else {
				layer.bindTooltip("<strong>"+nama_jenis_wilayah+"</strong><BR>Nama Wilayah Kelola: "+arr_data.nama_wikera,{permanent: false});
