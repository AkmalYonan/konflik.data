...
<?php
	...
	$lookup_tmp=lookup("cms_pp_category","kd_cat_pp","ur_cat_pp","parent_id=".$this->category,"order by order_num");
	
	$lookup_cat_pp=$lookup_tmp;
	if(count($lookup_tmp)>1):
		$lookup_cat_pp=$lookup_empty+$lookup_tmp;
	endif;
	$req=get_post();
	$cat=$req["cat"]?$req["cat"]:'';
    ...
    <div class="container" style="padding:0 30px;">
        <div class="row">
            <div class="col-sm-12">
                <div class="row">
                    <div class="col-md-4">
                      <h6 class="box-title" style="ext-transform:uppercase"><strong>HASIL PENELITIAN & ANALISIS</strong></h6>
                    </div>
                        ...
                        <!--
                          <div class="form-group">
                            <select name="sorter" class="form-control">
                               <option value="" class="small-opt"> Urutan Data</option>
                               <option value="desc" <?=($_GET['sorter']=="desc")?"selected":""?> class="small-opt"> Dari input Terbaru</option>
                               <option value="asc" <?=($_GET['sorter']=="asc")?"selected":""?> class="small-opt" >Dari Input Terlama</option>
              ...
              <div class="box-body table-responsive">
                <table class="table table-condensed small-font">
                    <thead>
                        ...
                        <th colspan="2" style="width:200px">Judul</th>
                        <th >Deskripsi</th>
                            	...
                            	<tr>
                                    ...
                                    <?php $gambar=$val['image']!=''?$val['image']:"noimg.jpg"; ?>
						 	        <td>
                                        ...
                                        <? else:?>
                                           <div style="border:1px solid grey;width:60px !important">
                                                <small><p><center> No <br />Cover</center></p></small>
                                           </div>
                                        <? endif;?>
                                    </td>
                                    <td rel="no_pp"><?=$val['no_pp'];?></td>
                                    
									<td><?php echo $val["about"]?></td>
                                    <td>
                                    <? if(cek_array($file_name_arr)):?>
                                    	<? foreach($file_name_arr as $xx=>$valx):?>
                                        <a href="<?=$this->config->item("dir_pp");?><?php echo $valx?>" title="<?=$valx;?>" data-toggle="tooltip"><i class="fa fa-cloud-download"></i> Unduh</a>
                                   <br>
										<? endforeach;?>
                                    <? else:?>
            ...
            <div class="box-body no-padding">
              <table class="table table-condensed">
				...
				<? foreach($lookup_cat_pp as $x=>$val):?>
                	<tr>
                    	<? if($cat==$x || count($lookup_cat_pp==1)):
