...
class pesisir extends Public_Controller {
	...
	function index($status=false){
		...
		*/
		...
		sum(CASE WHEN kode_jns_wikera='PPPBM' THEN 1 else 0 end) as pppbm,
		sum(luas) as jumlah_luas_wil,
		...
		$sql_wil_all = "select a.* from (select ".$as_kd1.","."count(idx) as jumlah from t_daftar_pppbm where ".$filter."  group by kd,kd2) a order by jumlah desc";
		...
		$arrWil2=$this->conn->GetAssoc($sql_wilsic);
			...
			$lookup_jenis_wikera=array("TORA"=>"TORA","PIAPS"=>"PS","HA"=>"HUTAN ADAT","PPPBM"=>"PPPBM");
		...
		$sql_wil_pppbm = "select a.* from (select ".$as_kd1.","."count(idx) as jumlah from ".$maintable." where ".$filter." and kode_jns_wikera='PPPBM' group by kd,kd2) a order by jumlah desc";
		$data["jml_wil_pppbm"]=$this->getWil($sql_wil_pppbm,array('#95c1de', '#177fde'),0);
		
		/* END DATA WILAYAH */
		// pre($sql_wil_pppbm);exit;
		
		
		/* DATA TERBARU */
		$tahapan=lookup("m_tahapan","kode","uraian");
		$data["tahapan"]=$tahapan;
		$top5_bydate  = $this->conn->GetAll("select idx, nama_wikera as judul,kode_jns_wikera,kode_tahapan, luas, tgl_kejadian from t_daftar_pppbm where ".$filter." order by tgl_kejadian desc limit 5");
		...
		//debug();
		$arrStatusTahapan=$this->conn->GetAll("select kode_tahapan,count(idx) as jumlah from t_daftar_pppbm ".$status_filter." group by kode_tahapan");
