...
class Cms{
    ...
    public function init_user(){
		$CI=& get_instance();
		$this->user_data=$_SESSION[$this->appname]["userdata"];//$CI->data['users']['user'];
        $this->user_group=$_SESSION[$this->appname]["groupdata"];//$CI->data['users']['groups'];
        $this->group_id=$_SESSION[$this->appname]["userdata"][$this->config->item('group_column')];
		//pre($this->group_brwa);
    }
    ...
    function has_none($module=false){
        ...
        if($module_id):
			 foreach($this->user_group as $x=>$group):
				 $right=$this->get_module_right($module_id, $group["id"]);
				 if($right>=$this->NONE_RIGHT):
					 return TRUE;
				 endif;
			 endforeach;
			/*$right=$this->get_module_right($module_id, $this->group_id);
            if($right>=$this->NONE_RIGHT):
                return TRUE;
            endif;*/
        endif;
    ...
    function has_view($module=false){
        ...
        foreach($this->user_group as $x=>$group):
           // $right=$this->get_module_right($module_id, $this->group_id);
			$right=$this->get_module_right($module_id, $group['group_id']);
			// pre($right);
    ...
    function get_module_right_max($module_path_or_url=false){
        ...
        if($module_id):
            foreach($this->user_group as $group):
                //$group_id=$group['group_id'];//$this->group_id;
                $data[]=$this->get_module_right($module_id, $group['group_id']);
            endforeach;
            return max($data);
        endif;
        return 0;
