...
class Lat_auth_Model extends CI_Model {
	...
	public function user($id = NULL)
	{
		...
		if ($id) {
			...
			$id = $_SESSION[$this->appname]["userdata"]["id"];
		}
		if ($id) {
			$user = $this->adodbx->GetRecord($this->tables["user"],"id=".$id);
			$user['groups_ids']=$this->get_users_groups($user['id'],true);
			$user['groups']=$this->get_users_groups($user['id']);
		}
		return $user;
	...
	//GROUPS
	function group_check($input = '',$exception_id=false)
	{
		if (empty($input))
		{
			return FALSE;
		...
		return $this->conn->GetOne("select count(id) from ".$this->tables["group"]." where name=?".$ex_sql,$data);
	}
	function groups($key=false,$val=false,$hide=true) {
		$sql_hide = ($hide)?" where status is null or status=0":"";
		$arrDB=$this->conn->GetAll("select * from ".$this->tables["group"].$sql_hide);
		if (!$key && !$val) {
		 	return $arrDB;
		}
		else {
	...
	{
		return $this->adodbx->GetRecord($this->tables["group"],"id=".($this->encrypt_status==TRUE?decrypt($id):$id));
	}
	function list_groups($where=false,$rows=false,$offset=false,$sort='',$dataColumn=false) {
		return $this->adodbx->search_record_by_limit_where($this->tables["group"],$where,$rows,$offset,$sort,$dataColumn);
	}
	public function add_group($data)
	{
		if ($this->group_check($data['name']))
		{
			set_message("error","Group sudah ada!");
			return FALSE;
		}
		...
		$this->conn->StartTrans();
		$this->adobx->Insert($this->tables["group"], $data);
		
		if ($this->conn->CompleteTrans()) {
			set_message("success","Tambah Group berhasil");
			return true;
		}
		return false;
	}
	...
	{
		$id=$this->encrypt_status==TRUE?decrypt($id):$id;
		if ($this->group_check($data['name'],$id))
		{
			set_message("error","Group sudah ada!");
			return FALSE;
		}
		...
		$this->conn->StartTrans();
		$this->adobx->Update($this->tables["group"], $data, "id=$id");
		
		if ($this->conn->CompleteTrans()) {
			set_message("success","Update Group berhasil");
		}

		return (isset($id)) ? $id : FALSE;
	}
	public function delete_group($id,$data)
	{
		$id=$this->encrypt_status==TRUE?decrypt($id):$id;
		if (!$this->group_check($data['name']))
		{
			set_message("error","Group tidak ditemukan!");
			return FALSE;
		}

		$this->conn->StartTrans();
		$this->adobx->Delete($this->tables["group"], "id=$id");
		if ($this->conn->CompleteTrans()) {
			set_message("success","Group berhasil dihapus");
		}

		return (isset($id)) ? $id : FALSE;
	}
	
	public function total_groups($where=false)
	{
		return $this->_total_where($this->tables["group"],$where,'id');
	}
