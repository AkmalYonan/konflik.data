...
class pasien extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->module_title="Daftar Tunggu Pasien";
		$this->tbl_idx="idx";
	 ...
	 function listview(){
		...
		$where[]	=	"status_rehab='1' and status_proses='RG'";
	...
	function get_fmd($id_pasien,$id_jari){
		...
		$data=$this->model_finger_foto->GetRecordData("idx_pasien=$id_pasien and id_jari=$id_jari and flag_default=1");
		if(cek_array($data)):
			print json_encode($data,true);
		else:
			print false;
