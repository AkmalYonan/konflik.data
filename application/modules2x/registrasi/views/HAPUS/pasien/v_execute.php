						  ...
						  <label>Tanggal Rencana Assesment<span class="asterix">&nbsp;*</span></label>
							<div class="input-group" >
										<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
										  <input type="text" style="width:220px;" id="tgl_assestment_selector" class="form-control input-date required" value="<?=date("d/m/Y",strtotime(date('Y')))?>" placeholder="dd/mm/yyyy" data-mindate="<?=date("d-m-Y",strtotime(date('Y')));?>"/>
...
<script>
	...
	$(function(){
		/*
		$('#tgl_registrasi_selector').datepicker().on("changeDate",function(ev){
			//alert(ev.date.format("isoDate"));
			$("#tgl_assesment_selector").val($(this).val());
			$("#tgl_assesment_selector").datepicker("update");
