...
class news extends Admin_Controller {
    ...
    function __file_upload($file_name,$name=false) {
		$cfolder = $this->config->item('dir_news_image');
		if (!is_dir($cfolder)) mkdir($cfolder);
		
		$folder = $this->config->item('dir_news_image');
		$data["process"]=true;
		if ($file_name) {
		
			// $fix_name = (($name)?$name:$file_name).substr($file_name,strrpos($file_name,"."));
			$fix_name = $file_name;
			$tmp_name = $this->config->item('dir_tmp_news_image').$file_name;
	...
	function add(){
		...
		if ($_SERVER['REQUEST_METHOD']=='POST') {
			...
			$config['upload_path']		=	$this->config->item("dir_news");
			...
			if($file['file_ext']==".jpg" or $file['file_ext']==".jpeg" or $file['file_ext']==".gif" or $file['file_ext']==".png"):
			$_data['image']=$file['file_name'];
			endif;
			
			$_data["created"]=$_POST['news_date'];
  ...
  function delete($idx=false){
		...
		if ($delete) {
			$data["delete"]=true;
			set_message("success","News Deleted");
			redirect("admin/news/");
		}
