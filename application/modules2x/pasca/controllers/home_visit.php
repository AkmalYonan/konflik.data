...
class home_visit extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->model_hm=new general_model("t_pasca_home_visit");
		$this->model_hm_history=new general_model("t_pasca_home_visit_history");
	 ...
	 function listview(){
        ...
        $sql=" select a.*,b.tgl_hm,b.hsl_hm,b.lampiran,b.keterangan,b.notlp_keluarga,b.nm_keluarga,b.nm_petugas,b.alamat as alamat_survey from ".$this->model->tbl." a
                left join ".$this->model_hm->tbl." b on a.idx=b.idx_pasien and b.status_data=1 where a.active_pasien=1
        ";
        ...
        //proses Entry Unit
		//$where[]="(status_rehab=2 and status_proses='RIRMEU')";
        ...
        $where[]="(status_rehab=3 and status_proses='PRRLPDHV')";
		...
		foreach($arrData as $k=>$v):
			for($i=1; $i<5; $i++):
				$last_idx	=	$this->conn->GetOne("select max(idx) from ".$this->model_hm_history->tbl." where idx_pasien='".$v['idx']."' and id_hm='$i'");
				$arrData[$k]['hv'.$i]	=	$this->model_hm_history->GetRecordData("idx='".$last_idx."'");
			endfor;
		endforeach;
	...
	function update_proses($id=false){
		...
		$act=$this->input->post("act")?$this->input->post("act"):"";
	   	if($act=="update"):
			...
			$config['upload_path']		=	$this->config->item("dir_hv");
			...
			if($data['status_pasien']=='SL'):
				...
				$data_update["status_proses"]="PRRLPDHV"; //detok
				$data_update["status_rehab"]=3; //rehab
				$data_update["status_rawat"]="PASCA"; //inap
				$data_update["kd_wilayah_propinsi"]=substr($kdwil, 0, 2);
				$data_mtr['pdhv']=0;
			...
			// debug();
			unset($data['idx']);
			$arrDtx=$this->model_hm->GetRecordData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
			if($arrDtx){
				$dataz['status_data']=0;
				$this->model_hm->UpdateData($dataz, "idx_pasien=$id");
				$data['status_data']=1;
				if($data['status_pasien']=='SL'){
					$data['status_data']=0;
				}
				$this->model_hm->DeleteData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
				$this->model_hm->InsertData($data);
			}
			$arrDtxHis=$this->model_hm_history->GetRecordData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
			$data['idx_parent']=$this->model_hm->GetLastId("idx");
			if($arrDtxHis){
				$dataz['idx_parent']=$this->model_hm->GetLastId("idx");
				$this->model_hm_history->UpdateData($dataz, "idx_pasien=$id and idx_assesment=$data[idx_assesment]");
				if($idx !=''){
					$this->model_hm_history->DeleteData("idx=$idx");
				}
				if($data['status_pasien']=='SL'){
					$data['status_data']=0;
					$this->model_hm_history->InsertData($data);
				}else{
					$data['status_data']=1;
					$this->model_hm_history->InsertData($data);
				}
			...
			// if($arrDtxHis){
				// $dataz['idx_parent']=$this->model_hm->GetLastId("idx");
				// $this->model_hm_history->UpdateData($dataz, "idx_pasien=$id");
				////if($idx !=''){
					// $this->model_hm_history->DeleteData("idx=$idx");
				////}
				// if($data['status_pasien']=='SL'){
			...
			$dataMonitoring['pdhv']=$data_mtr['pdhv'];
			if($data['id_hm']==1){
				$dataMonitoring['tgl_pdhv']=$data['tgl_kegiatan'];
			}
			$dataMonitoring['tgl_pdhv_selesai']=$data['tgl_selesai'];
	   ...
	   if($act=="create"):
			...
			if($arrDtx){
				...
				$this->model_hm->UpdateData($data, "idx=$idx");
			}else{
				$dataz['status_data']=0;
				$this->model_hm->UpdateData($dataz, "idx_pasien=$id");
				if($data['status_pasien']=='SL'){
	...
	function view_detail($id){
		...
		for($i=1; $i<5; $i++):
			$last_idx	=	$this->conn->GetOne("select max(idx) from ".$this->model_hm_history->tbl." where idx_pasien=$id and idx_assesment='$idx_assesment' and id_hm='$i'");
			$history[$i]	=	$this->model_hm_history->GetRecordData("idx='$last_idx'");
		endfor;
	...
	function view($id){
		...
		$data["pasien_hm_history"]=$this->model_hm_history->SearchRecordWhere("idx_pasien=$id and idx_assesment='$idx_assesment'");
		...
		$data["jns_kegiatan"]		=	$this->model_jenis_kegiatan->ListAll($this->jenis_kegiatan_filter);
		$data["data"]=$arrData;
		$data["data_proses"]=$this->model_hm->GetRecordData("idx_pasien=$id and idx_assesment='$idx_assesment'");
    ...
    function del($id){
       if($this->encrypt_status==TRUE):
        ...
        if($act=="delete"):
            $this->conn->StartTrans();
				...
				$this->model_hm->DeleteData("idx_pasien=$id");
	  ...
	  function get_array_idx(){
		$sql			=	"select idx_pasien from ".$this->model_hm->tbl;
	...
	public function delete_ajax(){
		...
		//Delete Session
		$criteria	=	"idx='".$idx."'";
		$this->model_hm_history->DeleteData($criteria);
		//End
		...
		$lastIdx	=	$this->conn->GetOne("select max(idx) from ".$this->model_hm_history->tbl.$criteria2);

		if($lastIdx):
			//Change status_data to 1
			$update_status_data['status_data']	=	1;
			$this->model_hm_history->UpdateData($update_status_data,"idx='".$lastIdx."'");
			//End

			$arr	=	$this->model_hm_history->GetRecordData("idx='".$lastIdx."'");
			...
			$this->model_hm->UpdateData($arrUpdate,$criteria3);
			//End
		else:
			$this->model_hm->DeleteData("idx_pasien='".$idx_pasien."'");
		endif;
