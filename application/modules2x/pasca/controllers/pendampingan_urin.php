...
class pendampingan_urin extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->model_pasien=new general_model("t_pasien_pasca_du");//
		$this->model_pasien_history=new general_model("t_pasien_pasca_du_history");//
		...
		$this->module_title="Kegiatan Pendampingan - Tes Urin";
		$this->tbl_idx="idx";
		$this->tbl_idx_pasien="idx_pasien";
		$this->tbl_sort="idx desc";
		$this->jenis_kegiatan_filter	=	"kategori='du'";
		$this->init_lookup();
	 ...
	 function listview(){
		...
		*/
		...
		$where[]="(status_rehab=3 and status_proses='PRRLPDTU')";
	...
	function add($id){
        ...
        if($act=="add"):
			...
			if($data['lampiran_name']):
				...
				$config['upload_path']		=	$this->config->item("dir_pendampingan_urin");
    ...
    function edit($id){
		...
		if(empty($act)):
			...
			$data["pasien_pasca_du_history"] =$this->model_pasien_history->SearchRecordWhere("idx_pasien='$id' and idx_assesment='$idx_assesment'");//
			...
			$data["data_proses"]			=$this->model_pasien->GetRecordData("idx_pasien=$id and idx_assesment='$idx_assesment'");
		...
		if($act=="update"):
			...
			if($data['status_program']=='SL'):
				...
				$data_update["status_proses"]="PRRLPDTU"; //detok
			...
			// if($arrDtx){
				...
				// if($data['status_program']=='SL'){
					// $data['status_data']=0;
				// }
				// $this->model_pasien->UpdateData($data, "idx=$id");
			// }
			...
			$dataMonitoring['pdtu']=$data_mtr['pdtu'];
			if($data['idx_tes']==1){
				$dataMonitoring['tgl_pdtu']=$data['tgl_kegiatan'];
			}
			$dataMonitoring['tgl_pdtu_selesai']=$data['tgl_selesai_program'];
		...
		if($act=="create"):
			...
			// $this->model_pasien_history->InsertData($data);
			// $this->model_pasien->InsertData($data, "{$this->tbl_idx_pasien}=$id");
			$data_update['tgl_kegiatan']=$data['tgl_kegiatan'];
			$data_update['status_pasien']=$data['status_program'];
			$data_update['tgl_selesai']=$data['tgl_selesai_program'];
			update_pasien_history_lanjut($data,$data_update);
    ...
    function del($id){
       if($this->encrypt_status==TRUE):
		...
		if($arrData['file']):
			unlink($this->config->item('dir_pendampingan_urin').$arrData['file']);
		endif;
	...
	function view_detail($id){
		...
		//$arrDataAsm=$this->model_assesment_summary->GetRecordData("idx_pasien=$id and active_pasien=1");//tiba tiba 0 ini lama
