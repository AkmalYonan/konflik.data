...
class pengembangan_diri extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->model_pasien=new general_model("t_pasien_pasca_pd");
		$this->model_pasien_history=new general_model("t_pasien_pasca_pd_history");
		...
		$this->module_title="Perkembangan Diri";
		$this->tbl_idx="idx";
		$this->tbl_idx_pasien="idx_pasien";
		$this->tbl_sort="idx desc";

		$this->jenis_kegiatan_filter	=	"kategori='pd'";
		$this->init_lookup();
	 ...
	 function listview(){
		...
		$where[]="(status_rehab=3 and status_proses='PRRIPD')";
	...
	function add($id){
        ...
        if($act=="add"):
			...
			if($data['lampiran_name']):
				...
				$config['upload_path']		=	$this->config->item("dir_pengembangan_diri");
    ...
    function view($id){
		...
		if(empty($act)):
			...
			$data["pasien_pd_history"]=$this->model_pasien_history->SearchRecordWhere("idx_pasien=$id and idx_assesment='$idx_assesment'");
		...
		if($act=="update"):
			...
			if($data['idx_pertemuan']==1){
				$data['tgl_kegiatan']=$data['tanggal'];
			...
			if($data['status_pasien']=='SL'):
				$data_update["status_proses"]="PRRIDK"; //entry unit
				...
				$data_update["status_proses"]="PRRIPD"; //detok
				$data_update["status_rehab"]=3; //rehab
				$data_update["status_rawat"]="PASCA"; //inap
				$data_update["kd_wilayah_propinsi"]=substr($kdwil, 0, 2);
				$data_mtr['pd']=0;
			...
			if($arrDtxHis){
				...
				$this->model_pasien_history->UpdateData($dataz, "idx_pasien=$id and idx_assesment=$data[idx_assesment]");
				if($idx !=''){
			...
			$dataMonitoring['pd']=$data_mtr['pd'];
			if($data['idx_pertemuan']==1){
				$dataMonitoring['tgl_pd']=$data['tgl_kegiatan'];
			}
			$dataMonitoring['tgl_pd_selesai']=$data['tgl_selesai'];
    ...
    function del($id){
       if($this->encrypt_status==TRUE):
		...
		if($arrData['file']):
			unlink($this->config->item('dir_pengembangan_diri').$arrData['file']);
		endif;
	...
	function view_detail($id){
		...
		$data['id']		=	$id_enc;
		$data["data"]	=	$arrData;
		$data["data_proses"]	=	$arrPasien;

		for($i=1; $i<3; $i++):
			$last_idx	=	$this->conn->GetOne("select max(idx) from ".$this->model_pasien_history->tbl." where idx_pasien=$id and idx_assesment='$idx_assesment' and idx_pertemuan='$i'");
	 ...
	 function edit_ori($id){
		...
		if($act=="update"):
			...
			//cek status_proses && update status
			if($data['status_pasien']=='SL'):
				$data_update["status_proses"]="PRRIDK";
