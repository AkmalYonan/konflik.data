...
class discharge extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->model_dsc_history=new general_model("t_pasca_discharge_history");
	 ...
	 function listview(){
        ...
        $sql=" select a.*,b.tgl_rujukan,b.jns_rujukan,b.status_pasien,b.keterangan,b.lampiran from ".$this->model->tbl." a
                left join ".$this->model_dsc->tbl." b on a.idx=b.idx_pasien and b.status_data=1 where a.active_pasien=1
        ";
        $table="($sql) a";
	...
	function update_proses($id=false){
		...
		$act=$this->input->post("act")?$this->input->post("act"):"";
	   	if($act=="update"):
			...
			$config['upload_path']		=	$this->config->item("dir_discharge");
			...
			if($data['status_pasien']=='SL'):
				...
				$data_update["status_proses"]="PRRIDR"; //detok
				$data_update["status_rehab"]=3; //rehab
				$data_update["status_rawat"]="PASCA"; //inap
				$data_update["kd_wilayah_propinsi"]=substr($kdwil, 0, 2);
				$data_mtr['dr']=0;
			...
			// debug();
			unset($data['idx']);
			$arrDtx=$this->model_dsc->GetRecordData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
			if($arrDtx){
				$dataz['status_data']=0;
				$this->model_dsc->UpdateData($dataz, "idx_pasien=$id");
				$data['status_data']=1;
				if($data['status_pasien']=='SL'){
					$data['status_data']=0;
				}
			...
			$arrDtxHis=$this->model_dsc_history->GetRecordData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
			if($arrDtxHis){
				$dataz['status_data']=0;
				$this->model_dsc_history->UpdateData($dataz, "idx_pasien=$id");
				$data['status_data']=1;
				if($data['status_pasien']=='SL'){
					$data['status_data']=0;
				}
				$this->model_dsc_history->InsertData($data);
			}
			// exit;
			...
			$data_update['tgl_kegiatan']=$data['tgl_rujukan'];
			...
			$dataMonitoring['dr']=$data_mtr['dr'];
			$dataMonitoring['tgl_dr']=$data['tgl_rujukan'];
			$dataMonitoring['tgl_dr_selesai']=$data['tgl_selesai'];
	...
	function view_detail($id){
		...
		$data["data"]=$arrData;
		$data["data_proses"]=$this->model_dsc->GetRecordData("idx_pasien=$id and idx_assesment='$idx_assesment'");
		$data['id']		=	$id_enc;
	...
	function view($id){
		...
		//pre($data["rehab_stat"]);pre($data["pasca_stat"]);exit;
