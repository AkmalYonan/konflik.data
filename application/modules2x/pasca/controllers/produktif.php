...
class produktif extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->model_pasien=new general_model("t_pasien_pasca_pp");
		$this->model_pasien_history=new general_model("t_pasien_pasca_pp_history");
		...
		$this->module_title="Kegiatan Produktif";
		$this->tbl_idx="idx";
		$this->tbl_idx_pasien="idx_pasien";
		$this->tbl_sort="idx desc";
		$this->jenis_kegiatan_filter	=	"kategori='pp'";
		$this->init_lookup();
	 ...
	 function listview(){
        ...
        $where[]="(status_rehab=3 and status_proses='PRRLPUKP')";
	...
	function add($id){
        ...
        if($act=="add"):
			...
			if($data['lampiran_name']):
				...
				$config['upload_path']		=	$this->config->item("dir_produktif");
    ...
    function view($id){
		...
		if(empty($act)):
			...
			$data["model_pasien_history"]=$this->model_pasien_history->SearchRecordWhere("idx_pasien=$id and idx_assesment='$idx_assesment'");
		...
		if($act=="update"):
			...
			if($data['status_pasien']=='SL'):
				...
				$data_update["status_proses"]="PRRLPUKP"; //detok
				$data_update["status_rehab"]=3; //rehab
				$data_update["status_rawat"]="PASCA"; //inap
				$data_update["kd_wilayah_propinsi"]=substr($kdwil, 0, 2);
				$data_mtr['pukp']=0;
			...
			$dataMonitoring['pukp']=$data_mtr['pukp'];
			$dataMonitoring['tgl_pukp']=$data['tgl_kegiatan'];
			$dataMonitoring['tgl_pukp_selesai']=$data['tgl_selesai'];
			// $arx=$this->model_pasien_assesment_history->GetRecordData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
			// if($arx['tgl_mulai_pasca']==''){
    ...
    function del($id){
       if($this->encrypt_status==TRUE):
		...
		if($arrData['file']):
			unlink($this->config->item('dir_produktif').$arrData['file']);
		endif;
