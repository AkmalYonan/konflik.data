...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    <li><a href="rehab/pasca_rehab"><i class="fa fa-child"></i> <?=$this->parent_module_title?></a></li>
                ...
                <table class="table table-bordered table-condensed small-font">
                    <thead valign="top">
                        <tr>
                            ...
                            <th>Jenis Kelamin</th>
                            <th>Tanggal<br /> Penerimaan<br /> Pasca Rehab</th>
							<th>Hasil Assesment</th>
                            <th>Keterampilan</th>
                            <th>Resume Rehabilitasi</th>
                            <th>Keterangan</th>
                            
                            <!--<th>Lampiran</th>-->
							<th>Tinjauan<br />Rencana<br />Terapi</th>
						...
						<?php if(cek_array($arrData)):?>
							...
							?>
                                <tr>
									...
									<td><?=$lookup_jns_org[($val['inst_pasca'])? $val['inst_pasca']:$val['inst_rujuk']]?><br />
										
										<? if(($val['inst_pasca']=='BNNP') || ($val['inst_pasca']=='BNNK')){?>
											<?=$lookup_bnnp[($val["rujuk_pasca"])? $val["rujuk_pasca"]:$val["rujuk_rehab"]]?>
										<? }else{?>
											<?=$lookup_inst[($val["rujuk_pasca"])? $val["rujuk_pasca"]:$val["rujuk_rehab"]]?>
										<? }?>
										
									</td>
                                    <td><?php echo $val["no_rekam_medis"];?></td>
                                    <td><?php echo $this->data_lookup["jenis_kelamin"][$val["jenis_kelamin"]];?></td>
                                    <td><?php echo ($val["tgl_penerimaan_pascarehab"])?date2indo($val["tgl_penerimaan_pascarehab"]):"<center>-</center>"?></td>
                                     <td>
										<?php if($val['hasil_assesment']): ?>
									 	<?php foreach($arr_hsl as $k=>$v):?> 
											<?=$k+1 .". ".$this->hsl_asses[$v];?> <br/>
										<? endforeach;?>
										<?php endif; ?>
									</td>
                                     <td><?php echo $val["ketrampilan"]?></td>
									 <td><?php echo $val["resume_rehab"]?></td>
									 <td><?php echo $val["keterangan"]?></td>
									 <!--<td><?php echo $val["lampiran"]?></td>-->
									  <td>
									  <?php
										if($val["tinjauan_rencana_terapi"]==1){
											echo "<span class='label label-success'>Sesuai</span>";
										}elseif($val["tinjauan_rencana_terapi"]==2){
											echo "<span class='label label-danger'>Tidak Sesuai</span>";
										}elseif($val["tinjauan_rencana_terapi"]==3){
                                     ...
                                     <td align="center">
                                        ...
                                        <!--
										<a class="btn btn-xs btn-default" href="<?=$this->module?>del/<?=$id?>" data-toggle='tooltip' title="Delete"><i class="fa fa-remove red"></i></a> 
                                        -->
									   </div>
                                    </td>
                                </tr>
