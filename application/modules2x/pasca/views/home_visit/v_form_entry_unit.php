...
<div class="row">
	<div class="col-sm-12">
		<table class="table table-striped table-condensed data_tbl">
			<?php if(count($pasien_hm_history)>0):?>
				<?php foreach($pasien_hm_history as $x=>$val): ?>
					<tr id="tr_<?=$val['idx']?>">
						<td align="center"><?php echo $x+1?></td>
						<td><?php echo ($val["tgl_hm"])?date2indo($val["tgl_hm"]):"<center>-</center>"?></td>
						<td align="center">Home Visit Ke-<?php echo $val["id_hm"]?></td>
...
<h4 class="heading title_form">Tambah Data Home Visit</h4>
<form id="frm" method="post" enctype="multipart/form-data" action="<?php echo $this->module;?>update_proses/<?php echo $id;?>">
    ...
    <div class="col-md-12">
		...
		<div class="row" style="margin-bottom:10px;">
			<div class="col-sm-6">
				<label for="header-home">Home Visit Ke :</label>
					...
					<?php 
						foreach($new_hm as $k=>$v){
							if($v != '')continue;
								$xcount[] = $v;
					?>
					<option value="<?=$k?>" <?=($data_proses['id_hm']==$k)?"selected":""?>>Home Visit Ke - <?=$k?></option>
					<? }?>
				</select>
			</div>
			...
			<div class="col-sm-6">
				...
				?>
				<label for="tgl_hm" id="id_kunjungan">Tanggal Home Visit</label>
		...
		<div class="row">
			<div class="col-sm-6">
                ...
                ?>
                <label for="hsl" id="hsl_kunjungan">Hasil Home Visit</label>
    	...
    	<div class="form-group">
                <label for="lokasi">Keterangan Home Visit</label>
		...
		<div class="form-group">
                <label for="nm_petugas">No Telp PJ/Keluarga</label>
				<input type="text" class="form-control" name="notlp_keluarga" id="notlp_keluarga" value="<?=$data_proses["notlp_keluarga"]?>" />
		</div>
		<div class="form-group">
		...
		<div class="form-group">
            ...
            <?php if($data_proses["lampiran"]){?>
				File: <a target="_blank" href="<?=$this->config->item("dir_hv").$data_proses["lampiran"]?>">Download</a>
			<?php } ?>
			</div>
		</div>
		<div class="form-group">
...
<script>
	$(document).ready(function(){
		$(".edit_data").on("click",function(){
			$(".title_form").text("Edit Data Home Visit");
				
			var string = $(this).data("ref");
			eval('var object='+string);
			
			<?php foreach($pasien_hm_history[0] as $k=>$v): ?>
			...
			if(lampiran){
				var href	=	'<a href="<?=$this->config->item("dir_hv")?>'+lampiran+'" target="_blank">Unduh Lampiran</a>';
		...
		$(".add_clean").on("click",function(){
			$(".title_form").text("Tambah Data Home Visit");
