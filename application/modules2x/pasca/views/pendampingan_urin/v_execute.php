...
<section class="content-header">
  <h1 class="hidden-xs">
    ...
    <small>Pasien Kegiatan Pendampingan Tes Urin</small>
  </h1>
  <ol class="breadcrumb">
				...
				<div class="row">
					...
					<div class="col-sm-6">
						<ul class="nav nav-tabs" role="tablist">
							 <li role="presentation" class="active"><a href="#homex" aria-controls="homex" role="tab" data-toggle="tab">Data Kegiatan Pendampingan - Tes Urin</a></li>
						</ul>
					...
					<div class="col-sm-12">
						<table class="table table-striped table-condensed data_tbl">
							<?php if(count($pasien_pasca_du_history)>0):?>
								<?php foreach($pasien_pasca_du_history as $x=>$val): 
								...
								?>
									...
									<tr id="tr_<?=$val['idx']?>">
										...
										<td align="center">
											<?php if($val['lampiran']): ?>
												<a href="<?=$this->config->item("dir_pendampingan_urin").$val['lampiran']?>" target="_blank">
										...
										<td align="center">
											<?php 
												foreach($val as $k=>$v): 
													$ref[]	=	$k.":'".$v."'";
												endforeach;
												$ref[]	=	"tgl_registrasi:'".date("Y-m-d",strtotime($val["tgl_kegiatan"]))."'";
												$ref[]	=	"tgl_registrasi_selector:'".date("d/m/Y",strtotime($val["tgl_kegiatan"]))."'";
												$data_ref	=	join(',',$ref);
											?>
						...
						<h4 class="heading title_form">Data Kegiatan Pendampingan - Tes Urin</h4>
						...
						<form id="frm" method="post" action="<?php echo $this->module;?><?=$act?>/<?=$id?>" enctype="multipart/form-data">
										...
										<div class="form-group">
											...
											<div class="input-group">
												<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
													<input type="text" id="tgl_kegiatan_selector" class="form-control input-date required" value="<?=$kegiatan_selector?>" placeholder="dd/mm/yyyy" />
													<input type="hidden" id="tgl_kegiatan" name="tgl_kegiatan" value="<?=$kegiatan?>" class="required" />
											</div>	
										</div>								
							...
							<div class="row">
								<div class="col-sm-6">
									...
									<div class="form-group">
										<label for="tanggal_input">Tes Urin</label>
										<select name="idx_tes" class="form-control" id="idx_tes">
							...
							<div class="row">
								<div class="col-sm-6">
									<div class="form-group">
										<label id="hasil_tes_label">Hasil Tes <?=$datax['idx_tes']?"Ke ".$datax['idx_tes']:"Ke 1"?></label>
							...
							<div class="form-group">
								...
								<textarea id="keterangan" name="keterangan" class="form-control" rows="3"><?=$datax['keterangan']?></textarea>
							</div>
							
							<div class="form-group">
								<label>Nama PJ/Keluarga</label>
								<input type="text" name="nama_pj_keluarga" id="nama_pj_keluarga" class="form-control" value="<?=$datax['nama_pj_keluarga']?>" />
							</div>
							
							<div class="form-group">
								<label>No Telepon PJ/Keluarga</label>
								<input type="text" name="no_telp_pj_keluarga" id="no_telp_pj_keluarga" class="form-control" value="<?=$datax['no_telp_pj_keluarga']?>" />
							</div>
							
							<div class="form-group">
								<label>Alamat PJ/Keluarga</label>
								<textarea name="alamat_pj_keluarga" id="alamat_pj_keluarga" class="form-control" rows="3"><?=$datax['alamat_pj_keluarga']?></textarea>
							</div>
							
							<div class="form-group">
								<label>Nama Pemantau</label>
								<input type="text" name="nama_petugas_tes" id="nama_petugas_tes" class="form-control" value="<?=$datax['nama_petugas_tes']?>" />
							</div>
							
							<div class="form-group">
									<label class="label_lampiran" for="nm_petugas">Lampiran</label>
									<input type="file" name="lampiran" id="lampiran"  />
									<?php if($datax["lampiran"]){?>
									File: <a target="_blank" href="<?=$this->config->item("dir_pendampingan_urin").$datax["lampiran"]?>">Download</a>
								<?php } ?>
							</div>
							<!--
							<div class="form-group">
...
<script>
	$(document).ready(function(){
		$(".edit_data").on("click",function(){
			$(".title_form").text("Edit Data Kegiatan Pendampingan - Tes Urin");
				
			var string = $(this).data("ref");
			eval('var object='+string);
			
			<?php foreach($pasien_pasca_du_history[0] as $k=>$v): ?>
			<?php if(($k!=="lampiran") && ($k!=="tgl_selesai")): ?>
			$("#<?=$k?>").val(object.<?=$k?>);		
			<?php endif; ?>
			<?php endforeach; ?>
			
			$("#idx_tes").val(object.idx_tes).change();
			...
			if(lampiran){
				var href	=	'<a href="<?=$this->config->item("dir_pendampingan_urin")?>'+lampiran+'" target="_blank">Unduh Lampiran</a>';
		...
		$(".add_clean").on("click",function(){
			$(".title_form").text("Tambah Data Kegiatan Pendampingan - Tes Urin");
			
			$("#idx").val('');
			$("#idx_tes").val("1").change();
			$("#radio_plus").prop("checked",true);
			$("#radio_minus").prop("checked",false);
			$("#keterangan").val("");			
			$("#nama_pj_keluarga").val("");
			$("#no_telp_pj_keluarga").val("");
			$("#nama_petugas_tes").val("");
			$("#alamat_pj_keluarga").val("");
			$("#tgl_registrasi").val('<?=date("Y-m-d")?>');
			$("#tgl_registrasi_selector").val('<?=date("d/m/Y")?>');
			$(".label_lampiran").text("");
