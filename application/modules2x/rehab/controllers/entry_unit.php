...
class entry_unit extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->model_eu_history=new general_model("t_pasien_entry_unit_history");
	 ...
	 function listview(){
        ...
        $sql=" select a.*,b.tgl_kegiatan,b.jangka_waktu,b.jangka_waktu_satuan,b.tgl_mulai,b.tgl_selesai,b.kegiatan,b.lampiran,b.status_pasien from ".$this->model->tbl." a
                left join ".$this->model_eu->tbl." b on a.idx=b.idx_pasien and b.status_data=1 where a.active_pasien=1
        ";
	...
	function update_proses($id=false){
		...
		$act=$this->input->post("act")?$this->input->post("act"):"";
	   	if($act=="update"):
			...
			$config['upload_path']		=	$this->config->item("dir_eu");
			...
			if($data['status_pasien']=='SL'):
				$data_update["status_proses"]="RIRSPP"; //entry unit
				...
				$data_update["status_proses"]="RIRMEU"; //detok
				$data_update["status_rehab"]=2; //rehab
				$data_update["status_rawat"]="INAP"; //inap
				$data_update["inst_rujuk"]=$inst_rujuk;
				$data_update["rujuk_rehab"]=$rujuk_rehab;
			...
			// debug();
			unset($data['idx']);
			$arrDtx=$this->model_eu->GetRecordData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
			if($arrDtx){
				$dataz['status_data']=0;
				$this->model_eu->UpdateData($dataz, "idx_pasien=$id");
				$data['status_data']=1;
				if($data['status_pasien']=='SL'){
					$data['status_data']=0;
				}
				$this->model_eu->DeleteData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
				$this->model_eu->InsertData($data);
			}
			$arrDtxHis=$this->model_eu_history->GetRecordData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
			$data['idx_parent']=$this->model_eu->GetLastId("idx");
			if($arrDtxHis){
				$dataz['idx_parent']=$this->model_eu->GetLastId("idx");
				$this->model_eu_history->UpdateData($dataz, "idx_pasien=$id and idx_assesment=$data[idx_assesment]");
				if($idx !=''){
					$this->model_eu_history->DeleteData("idx=$idx");
				}
				if($data['status_pasien']=='SL'){
					$data['status_data']=0;
					$this->model_eu_history->InsertData($data);
				}else{
					$data['status_data']=1;
					$this->model_eu_history->InsertData($data);
				}
			...
			// if($arrDtxHis){
				// $dataz['idx_parent']=$this->model_eu->GetLastId("idx");
				// $this->model_eu_history->UpdateData($dataz, "idx_pasien=$id");
				////if($idx !=''){
					// $this->model_eu_history->DeleteData("idx=$idx");
				////}
				// if($data['status_pasien']=='SL'){
			...
			//monitoring
			$dataMonitoring['eu']=$data_mtr['eu'];
			if($data['pertemuan_ke']==1){
				$dataMonitoring['tgl_eu']=$data['tgl_kegiatan'];
			}
			$dataMonitoring['tgl_eu_selesai']=$data['tgl_selesai'];
	   ...
	   if($act=="create"):
			...
			if($arrDtx){
				...
				$this->model_eu->UpdateData($data, "idx=$idx");
	...
	function edit($id,$idx_eu){
        if($this->encrypt_status==TRUE):
            $id_enc=$id;
            $id=decrypt($id);
			$idx_eu=decrypt($idx_eu);
        endif;
		...
		// $data["data_proses"]=$this->model_eu->GetRecordData("idx_pasien=$id and idx_assesment='$idx_assesment'");
		$data["data_proses"]=$this->model_eu_history->GetRecordData("idx='$idx_eu'");
		$data["pasien_eu_history"]=$this->model_eu_history->SearchRecordWhere("idx_pasien=$id and idx_assesment='$idx_assesment'");
	...
	function detail($id){
		...
		for($i=1; $i<5; $i++):
			$last_idx	=	$this->conn->GetOne("select max(idx) from ".$this->model_eu_history->tbl." where idx_pasien='$id' and idx_assesment='$idx_assesment' and pertemuan_ke='$i'");

			$history[$i]=	$this->model_eu_history->GetRecordData("idx='$last_idx'");
		endfor;
	...
	public function delete_ajax(){
		...
		//Delete Session
		$criteria	=	"idx='".$idx."'";
		$this->model_eu_history->DeleteData($criteria);
		//End
		...
		$lastIdx	=	$this->conn->GetOne("select max(idx) from ".$this->model_eu_history->tbl.$criteria2);

		if($lastIdx):

		//Change status_data to 1
			$update_status_data['status_data']	=	1;
			$this->model_eu_history->UpdateData($update_status_data,"idx='".$lastIdx."'");
			//End

			$arr	=	$this->model_eu_history->GetRecordData("idx='".$lastIdx."'");
			...
			$this->model_eu->UpdateData($arrUpdate,$criteria3);
			//End
		else:
			$this->model_eu->DeleteData("idx_pasien='".$idx_pasien."'");
		endif;
