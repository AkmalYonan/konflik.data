...
class terapi_simtomatik extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->model_ts=new general_model("t_pasien_terapi_simtomatik");
		$this->model_ts_history=new general_model("t_pasien_terapi_simtomatik_history");
		...
		$this->module_title="Terapi Simtomatik";
		$this->tbl_idx="idx";
	 ...
	 function listview(){
        ...
        $sql=" select a.*,b.tgl_kegiatan,b.jangka_waktu,b.jangka_waktu_satuan,b.tgl_mulai,b.tgl_selesai,b.kegiatan,b.jenis_kegiatan,b.lampiran,b.status_pasien from ".$this->model->tbl." a
                left join ".$this->model_ts->tbl." b on a.idx=b.idx_pasien and b.status_data=1 where a.active_pasien=1
        ";
		...
		$where[]="(status_rehab=2 and status_proses='RJTS')";
	...
	function update_proses($id=false){
		...
		$act=$this->input->post("act")?$this->input->post("act"):"";
	   	...
	   	if($act=="update"):
			...
			$config['upload_path']		=	$this->config->item("dir_ts");
			...
			if($data['status_pasien']=='SL'):
				...
				$data_update["status_proses"]="RJTS"; //detok
			...
			// debug();
			unset($data['idx']);
			$arrDtx=$this->model_ts->GetRecordData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
			if($arrDtx){
				$dataz['status_data']=0;
				$this->model_ts->UpdateData($dataz, "idx_pasien=$id");
				$data['status_data']=1;
				if($data['status_pasien']=='SL'){
					$data['status_data']=0;
				}
				$this->model_ts->DeleteData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
				$this->model_ts->InsertData($data);
			}
			$arrDtxHis=$this->model_ts_history->GetRecordData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
			$data['idx_parent']=$this->model_ts->GetLastId("idx");
			if($arrDtxHis){
				$dataz['idx_parent']=$this->model_ts->GetLastId("idx");
				$this->model_ts_history->UpdateData($dataz, "idx_pasien=$id and idx_assesment=$data[idx_assesment]");
				if($idx !=''){
					$this->model_ts_history->DeleteData("idx=$idx");
				}
				if($data['status_pasien']=='SL'){
					$data['status_data']=0;
					$this->model_ts_history->InsertData($data);
				}else{
					$data['status_data']=1;
					$this->model_ts_history->InsertData($data);
				}
			...
			// if($arrDtxHis){
				// $dataz['idx_parent']=$this->model_ts->GetLastId("idx");
				// $this->model_ts_history->UpdateData($dataz, "idx_pasien=$id");
				////if($idx !=''){
					// $this->model_ts_history->DeleteData("idx=$idx");
				////}
				// if($data['status_pasien']=='SL'){
			...
			$dataMonitoring['ts']=$data_mtr['ts'];
			if($data['pertemuan_ke']==1){
				$dataMonitoring['tgl_ts']=$data['tgl_kegiatan'];
			}
			$dataMonitoring['tgl_ts_selesai']=$data['tgl_selesai'];
	   ...
	   if($act=="create"):
			...
			if($arrDtx){
				...
				$this->model_ts->UpdateData($data, "idx=$idx");
			}else{
				$dataz['status_data']=0;
				$this->model_ts->UpdateData($dataz, "idx_pasien=$id");
				if($data['status_pasien']=='SL'){
	...
	function edit($id,$idx_pt){
		...
		// $data["data_proses"]=$this->model_ts->GetRecordData("idx_pasien=$id and idx_assesment='$idx_assesment'");
		$data["data_proses"]=$this->model_ts_history->GetRecordData("idx='$idx_pt'");

		$data["model_ts_history"]=$this->model_ts_history->SearchRecordWhere("idx_pasien=$id and idx_assesment='$idx_assesment'");
	...
	function detail($id){
		...
		for($i=1; $i<5; $i++):
			$last_idx	=	$this->conn->GetOne("select max(idx) from ".$this->model_ts_history->tbl." where idx_pasien='$id' and idx_assesment='$idx_assesment' and pertemuan_ke='$i'");

			$history[$i]=	$this->model_ts_history->GetRecordData("idx='$last_idx'");
		endfor;
	...
	public function delete_ajax(){
		...
		//Delete Session
		$criteria	=	"idx='".$idx."'";
		$this->model_ts_history->DeleteData($criteria);
		//End
		...
		$lastIdx	=	$this->conn->GetOne("select max(idx) from ".$this->model_ts_history->tbl.$criteria2);

		if($lastIdx):
			//Change status_data to 1
			$update_status_data['status_data']	=	1;
			$this->model_ts_history->UpdateData($update_status_data,"idx='".$lastIdx."'");
			//End

			$arr	=	$this->model_ts_history->GetRecordData("idx='".$lastIdx."'");
			...
			$this->model_ts->UpdateData($arrUpdate,$criteria3);
			//End
		else:
			$this->model_ts->DeleteData("idx_pasien='".$idx_pasien."'");
		endif;
