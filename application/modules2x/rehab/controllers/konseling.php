...
class konseling extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->model_kl=new general_model("t_pasien_konseling");
		$this->model_kl_history=new general_model("t_pasien_konseling_history");
	 ...
	 function listview(){
        ...
        $sql=" select a.*,b.tgl_kegiatan,b.jangka_waktu,b.jangka_waktu_satuan,b.tgl_mulai,b.tgl_selesai,b.kegiatan,b.lampiran,b.keterangan,b.status_pasien from ".$this->model->tbl." a
                left join ".$this->model_kl->tbl." b on a.idx=b.idx_pasien and b.status_data=1 where a.active_pasien=1
        ";
		...
		if($this->user_prop):
			if($this->user_kab!=='00'):
				$rujuk_rehab	=	$this->user_prop.$this->user_kab;
				$where[]="(inst_rujuk='RJKL' and rujuk_rehab='".$rujuk_rehab."')";
			else:
				$rujuk_rehab	=	$this->user_prop;
				$where[]="(inst_rujuk='RJKL' and rujuk_rehab='".$rujuk_rehab."')";
			endif;
		endif;
		*/
		...
		$where[]="(status_rehab=2 and status_proses='RJKL')";
	...
	function update_proses($id=false){
		...
		$act=$this->input->post("act")?$this->input->post("act"):"";
	   	if($act=="update"):
			...
			$config['upload_path']		=	$this->config->item("dir_kl");
			...
			if($arrDtx){
				...
				$this->model_kl->DeleteData("idx_pasien=$id and idx_assesment=$data[idx_assesment]");
			...
			$data['idx_parent']=$this->model_kl->GetLastId("idx");
			if($arrDtxHis){
				$dataz['idx_parent']=$this->model_kl->GetLastId("idx");
				$this->model_kl_history->UpdateData($dataz, "idx_pasien=$id and idx_assesment=$data[idx_assesment]");
				if($idx !=''){
					$this->model_kl_history->DeleteData("idx=$idx");
				}
				if($data['status_pasien']=='SL'){
					$data['status_data']=0;
			...
			$dataMonitoring['kl']=$data_mtr['kl'];
			if($data['pertemuan_ke']==1){
				$dataMonitoring['tgl_kl']=$data['tgl_kegiatan'];
			}
			$dataMonitoring['tgl_kl_selesai']=$data['tgl_selesai'];
			// $dataMonitoring['tgl_mulai_rehab']=$data['tgl_kegiatan'];
	...
	function edit($id,$idx_pt){
		...
		$data["data_proses"]=$this->model_kl_history->GetRecordData("idx='$idx_pt'");
		// $data["data_proses"]=$this->model_kl->GetRecordData("idx_pasien=$id and idx_assesment='$idx_assesment'");
		$data["model_kl_history"]=$this->model_kl_history->SearchRecordWhere("idx_pasien=$id and idx_assesment='$idx_assesment'");
	...
	function detail($id){
		...
		for($i=1; $i<9; $i++):
			$last_idx	=	$this->conn->GetOne("select idx from ".$this->model_kl_history->tbl." where idx_pasien='$id' and idx_assesment='$idx_assesment' and pertemuan_ke='$i'");

			$history[$i]=	$this->model_kl_history->GetRecordData("idx='$last_idx'");
		endfor;
	...
	//------------------------------------- backup ------------------------
	//jika selesai curenr status
	function update_proses_backup($id=false){
		if($this->encrypt_status==TRUE):
			$id_enc=$id;
			$id=decrypt($id);
		endif;
		...
		$act=$this->input->post("act")?$this->input->post("act"):"";
	   	if($act=="update"):
			...
			if($data['status_pasien']=='SL'):
				...
				$data_update["status_proses"]="RJKL"; //detok
				$data_update["status_rehab"]=1; //rehab
				$data_update["status_rawat"]="JALAN"; //inap
				$data_mtr['kl']=0;
			endif;
			if(!empty($data["kd_wilayah"])):
	...
	function update_proses_ori($id=false){
		//walaupun belum selesai curent status
		if($this->encrypt_status==TRUE):
			$id_enc=$id;
			$id=decrypt($id);
		endif;
	   ...
	   if($act=="create"):
			...
			//cek status_proses && update status
			if($data['status_pasien']=='SL'):
				$data_update["status_proses"]="RJKL"; //entry unit
				$data_update["status_rehab"]=2; //rehab
				$data_update["status_rawat"]="JALAN"; //inap
				$data_mtr['kl']=1;
			else:
				$data_update["status_proses"]="RJ"; //detok
				$data_update["status_rehab"]=1; //rehab
				$data_update["status_rawat"]="JALAN"; //inap
				$data_mtr['kl']=0;
			endif;
	...
	public function delete_ajax(){
		...
		//Delete Session
		$criteria	=	"idx='".$idx."'";
		$this->model_kl_history->DeleteData($criteria);
		//End
		...
		$lastIdx	=	$this->conn->GetOne("select max(idx) from ".$this->model_kl_history->tbl.$criteria2);

		if($lastIdx):
			//Change status_data to 1
			$update_status_data['status_data']	=	1;
			$this->model_kl_history->UpdateData($update_status_data,"idx='".$lastIdx."'");
			//End

			$arr	=	$this->model_kl_history->GetRecordData("idx='".$lastIdx."'");
			...
			$this->model_kl->UpdateData($arrUpdate,$criteria3);
			//End
		else:
			$this->model_kl->DeleteData("idx_pasien='".$idx_pasien."'");
		endif;
