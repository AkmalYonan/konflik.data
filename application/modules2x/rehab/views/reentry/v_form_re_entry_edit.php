...
<form id="frm" method="post" enctype="multipart/form-data" action="<?php echo $this->module;?>update_proses/<?php echo $id;?>">
    ...
    <div class="col-md-12">
		...
		<div class="form-group">
			<div class="row">
				...
				<div class="col-md-4 tgl_selesai_program">
					...
					<div class="input-group">
						<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
						<input type="text" id="tgl_selesai_program_selector" class="input-sm form-control input-date required" value="<?=date("d/m/Y",strtotime($tgl_selesai))?>" placeholder="dd/mm/yyyy"/>
						 <input type="hidden" id="tgl_selesai_program" name="tgl_selesai" value="<?=date("Y-m-d",strtotime($tgl_selesai));?>" class="required" />
					</div><!-- end input group -->
				</div>
			</div>
		</div><!-- end form-->
		...
		<h4 class="heading">Daftar Kegiatan Re-Entry</h4>
						...
						?>
							<tr>
								...
								<td><?php echo $val["jenis_kegiatan"]?></td>
								 ...
								 <td align="center">
									<?php if($val['lampiran']): ?>
										<a href="<?=$this->config->item("dir_re").$val['lampiran']?>" target="_blank">
