...
class ketersediaan_balai extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->parent_module_title="Ketersediaan";
		$this->module_title="Balai";
		$this->tbl_idx="idx";
	 ...
	 function listview(){
		...
		$sql	=	"
					...
					a.nama_instansi,
					a.kd_instansi,
					a.jenis_tempat_rehab,
					a.jml_laki,
					a.jml_perempuan,
					sum(case when b.inst_rujuk='BL' and b.active_pasien='1' then 1 else 0 end) as pasien_masuk,
					sum(case when b.inst_rujuk='BL' and b.inst_pasca is not null and b.active_pasien='1' then 1 else 0 end) as pasien_keluar,
					year(b.tgl_registrasi) as tahun
					
					from ".$this->model->tbl." a
					
					left join t_pasien b
					on a.kd_instansi=b.rujuk_rehab
					
					group by a.idx,a.kd_instansi,year(b.tgl_registrasi)					
					";
