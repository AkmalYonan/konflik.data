...
class pasien extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->model_pasien=$this->load->model("data_pasien/pasien_model");
		$this->main_layout="admin_lte_layout/main_layout";
		$this->parent_module_title="PENDAFTARAN";
		...
		$this->lookup_proses	=	lookup("m_proses_rehab","kd_status_proses","ur_proses","","order by order_num");
	 }
	 ...
	 function add(){
        ...
        if($act=="create"):
			...
			$cek_nik	=	$this->pasien_model->CekNik($data['nik']);
			
			if($cek_nik):
				
				$ok	=	0;
				
				$this->msg_fail="Tidak Dapat Menambah Data, Pasien Dengan NIK ".$data['nik']." Sudah Terdaftar";
    ...
    function edit($id){
		...
		if($act=="update"):
			...
			$ok=$this->conn->CompleteTrans();
			$this->_proses_message($ok,$this->module."add/",$this->module."add");
	 ...
	 function add_pasien_baru(){
        ...
        if($act=="create"):
			...
			if($cek_nik):
				...
				$this->_proses_message($ok,$this->module."add/",$this->module."add/$nik/$no_rekam_medis");
				...
				$ok=$this->conn->CompleteTrans();
				$this->_proses_message($ok,$this->module."add/",$this->module."$nik/$no_rekam_medis");
	...
	public function cek_data_pasien(){
		...
		$nik			=	$datax['nik'];
		$no_rekam_medis	=	$datax['no_rekam_medis'];
		
		$cekDataPasien	=	$this->model_pasien->CekDataPasien($nik);
		
		$cek_active	=	$this->model_pasien->CekPasienActive($cekDataPasien);
		
		if(!$no_rekam_medis){		
			if($cekDataPasien):								
				if($cek_active):					
					$arr	=	$this->model->GetRecordData("idx='".$cek_active."'");
					$data['ver']=$datax;
					$data['data']=$arr;
					$data['remove_hide']	=	1;	
					$data['label_warning']	=	1;
					$this->_render_page($this->module."v_add",$data,true);				
				else:
					$arr	=	$this->model->GetRecordData("idx='".$cekDataPasien."'");
					$data['data']	=	$arr;
					$this->_render_page($this->module."v_execute",$data,true);
				endif;
			...
			$ver_no_rekam_medis	=	$this->model_pasien->VerifikasiNoRekamMedis($no_rekam_medis);
			
			if(!$ver_no_rekam_medis){
			
				$cek_no_rekam_medis	=	$this->model_pasien->CekNoRekamMedis($cekDataPasien,$no_rekam_medis);
				
				if($cek_no_rekam_medis):
					
					$cek_active	=	$this->model_pasien->CekPasienActive($cek_no_rekam_medis);
					...
					if($cek_active):
						...
						$arr	=	$this->model->GetRecordData("idx='".$cek_no_rekam_medis."'");
						$data['data']	=	$arr;
						$this->_render_page($this->module."v_execute",$data,true);
					endif;					
				...
				$arr	=	$this->model->GetRecordData("no_rekam_medis='".$ver_no_rekam_medis."'");
				
				$data['ver']=$datax;
				$data['data']=$arr;
				$data['remove_hide']	=	1;
				$data['label_warning']	=	3;
