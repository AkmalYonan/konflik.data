...
/**
 * Formats a numbers abbreviation
 *
 * @access	public
 * @param	mixed	// will be cast as int
 * @return	string
...
{
	function curr_format($num, $precision = 2)
	{
		$CI =& get_instance();
		$CI->lang->load('number');
		...
		if ($num >= 1000000000000)
		{
			...
			$unit = $CI->lang->line('trillion_abbr');
			return number_format($num, $precision,",",".").' '.$unit;
		...
		elseif ($num >= 1000000000)
		{
			...
			$unit = $CI->lang->line('billion_abbr');
			return number_format($num, $precision,",",".").' '.$unit;
		}
		elseif ($num >= 1000000)
		{
			$num = round($num / 1000000, $precision);
			$unit = $CI->lang->line('million_abbr');
			return number_format($num, $precision,",",".").' '.$unit;
		}
		else
		{
